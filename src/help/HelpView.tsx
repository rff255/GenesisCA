      ...
      <div className={styles.content} ref={contentRef}>
        ...
        <section id="help-simulator" className={styles.section}>
          ...
          <ul className={styles.list}>
            ...
            <li><strong>Screenshot</strong> &mdash; Save a PNG of the current frame. The dropdown next to the camera button chooses the <strong>scope</strong> &mdash; <strong>simulation</strong> (crops to the area of interest, no letterbox margins) or <strong>current view</strong> (the whole canvas as shown). Matches display resolution. In 3D the scene fills the frame, so the scope picker is hidden (there are no margins to crop).</li>
            <li><strong>Record</strong> &mdash; Click the red record button in the transport bar, play the simulation, then click stop to encode and download. The dropdown next to the button chooses both the <strong>format</strong> and the <strong>capture scope</strong>: <strong>WebM</strong> (smaller files, no 256-colour limit; needs a browser with WebCodecs &mdash; recent Chrome / Edge / Firefox) or <strong>GIF</strong> (256 colours, max 512&nbsp;px), each in two scopes &mdash; <strong>simulation</strong> (default; crops to the area of interest, removing the empty margins between the side panels) or <strong>current view</strong> (records the whole canvas exactly as shown, margins / pan / zoom included). Both scopes work uniformly for every model type and compile target (2D grid, grid&nbsp;+&nbsp;agents, agents-only, 3D). The framing is locked when you press record, so pan / zoom mid-recording won&apos;t change the frame size. In 3D the scene fills the frame, so only the format is offered (there are no margins to crop).</li>
