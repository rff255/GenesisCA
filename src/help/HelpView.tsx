      ...
      <div className={styles.content} ref={contentRef}>
        ...
        <section id="help-simulator" className={styles.section}>
          ...
          <ul className={styles.list}>
            ...
            <li><strong>Where the capture controls live</strong> &mdash; screenshot and recording are <em>output</em>, so they sit in their own cluster at the <strong>bottom-right of the canvas, directly beneath the stats readout</strong> &mdash; not on the transport bar, which is about simulation <em>time</em> (play / pause / step / reset / speed). The cluster is <code>&#128247;</code> (screenshot) &middot; <code>&#9210;</code> (record) &middot; a <strong>settings chip</strong> reading <code>format &middot; area &middot; quality</code> (e.g. <code>WebM &middot; sim &middot; Std</code>). Hover or click the chip to open the capture settings; every choice there is a two-state switch, and a setting that does not apply is <em>greyed in place with the reason</em> rather than disappearing (GIF has no quality / overload options; a 3D scene fills the frame so it has no separate area).</li>
            <li><strong>Screenshot</strong> &mdash; Save a PNG of the current frame. <strong>Screenshot area</strong> in the chip&apos;s popover chooses the <strong>scope</strong> &mdash; <strong>simulation</strong> (the whole grid / world framed to fit, independent of your zoom / pan) or <strong>current view</strong> (the display canvas as shown, with zoom / pan). In 3D the scene fills the frame, so the choice is greyed out.</li>
            <li><strong>Record</strong> &mdash; Click the red record button in the capture cluster, play the simulation, then click stop to save and download. The chip&apos;s popover chooses both the <strong>format</strong> and the <strong>capture area</strong>: <strong>WebM</strong> (smaller files, no 256-colour limit; needs a browser with WebCodecs &mdash; recent Chrome / Edge / Firefox) or <strong>GIF</strong> (256 colours, max 512&nbsp;px), each in two scopes &mdash; <strong>simulation</strong> (default; the whole grid / world framed to fit, <em>independent of your zoom / pan</em> &mdash; so it captures the entire simulation no matter how far you&apos;ve zoomed in to inspect a part, with no empty margins) or <strong>current view</strong> (records the display canvas exactly as shown, with your zoom / pan and any margins). Both scopes work for every model type and compile target (2D grid, grid&nbsp;+&nbsp;agents, agents-only, 3D). In 3D the scene fills the frame, so the area choice is greyed out. <em>Note:</em> in the simulation scope, agents are drawn as plain circles + bonds &mdash; use the current-view scope for a WYSIWYG capture of agent sprites, metaballs, or glow.</li>
            <li><strong>Long recordings</strong> &mdash; a <strong>WebM</strong> recording is encoded <em>as you record</em>, so only the compressed video accumulates rather than every raw frame: recordings can run many times longer before memory runs out, and Stop is a quick save instead of a long encode. Two consequences worth knowing. First, on a dense, fast-changing model the encoder is the slow part &mdash; the simulation will run slower while recording, and if it still cannot keep up the recorder <em>skips</em> frames rather than piling them up (see <strong>On overload</strong> below); any skipped frames are shown next to the REC counter (<code>REC 1200f &middot; 37 dropped</code>) and the video still plays at the chosen frame rate, just with those generations missing. Second, the <strong>FPS</strong> value in effect when you press Record is the frame rate written into the file, so changing FPS mid-recording no longer retimes the whole video. <strong>GIF</strong> keeps every frame in memory (its palette needs the raw pixels), and is limited to 256 colours and 512&nbsp;px &mdash; it remains a short-clip format; use WebM for anything long.</li>
            <li><strong>Recording quality</strong> (WebM only) &mdash; <strong>Standard</strong> (the default) writes a keyframe every 30 frames: measured <strong>~6&times; smaller files and ~3&times; faster encoding</strong> on dense content, which also means the simulation runs closer to full speed while recording. The trade is that a player has to decode from the last keyframe, so scrubbing lands on 30-frame boundaries. <strong>Archival</strong> makes every frame a keyframe: each frame is independently decodable (ideal for frame-by-frame analysis and scrub-exact playback) and nothing from interframe prediction can bleed across previously-stable regions &mdash; at roughly 6&times; the file size and 3&times; the encoding time. A skipped frame is safe in <em>both</em> modes: frames are skipped before they reach the encoder, so the video is simply missing those generations &mdash; it is never corrupted.</li>
            <li><strong>On overload</strong> (WebM only) &mdash; what gives when the encoder cannot keep up. <strong>Skip frames</strong> (the default) keeps the simulation at full speed and leaves the frames it could not encode out of the video. <strong>Never skip</strong> encodes every captured frame and instead holds the <em>simulation</em> back until the encoder catches up &mdash; the run gets slower (measured ~5&times; on a dense model) but the video loses nothing. While it is waiting, the stats overlay says <code>&#9203; waiting for encoder</code>, so a deliberately slowed run is never mistaken for a freeze; if the encoder ever stalls outright the recording falls back to skipping and tells you. Format, area, quality and this setting are all <strong>fixed at the moment you press Record</strong> &mdash; while a recording runs, the chip greys out and keeps showing the configuration in force, and the record button becomes <code>&#9209; 128</code> (frames captured, plus any skipped and <code>&#9203;</code> while the simulation is being held back).</li>
            <li><strong>Recording resolution</strong> &mdash; recordings are capped at 912&nbsp;px on the long edge in 2D and 1280&nbsp;px in 3D (screenshots are not capped and keep the full display resolution). The 2D figure is chosen so the encoder can use full-resolution colour (4:4:4), which keeps single-cell colour boundaries crisp; at the larger 3D size only the subsampled 4:2:0 mode is usable, so fine colour detail is slightly softer there in exchange for far more spatial detail.</li>
