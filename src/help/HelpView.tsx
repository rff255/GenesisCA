      ...
      <div className={styles.content} ref={contentRef}>
        ...
        <section id="help-simulator" className={styles.section}>
          ...
          <ul className={styles.list}>
            ...
            <li><strong>On overload</strong> (WebM only) &mdash; what gives when the encoder cannot keep up. <strong>Skip frames</strong> (the default) keeps the simulation at full speed and leaves the frames it could not encode out of the video. <strong>Never skip</strong> encodes every captured frame and instead holds the <em>simulation</em> back until the encoder catches up &mdash; the run gets slower (measured ~5&times; on a dense model) but the video loses nothing. While it is waiting, the stats overlay says <code>&#9203; waiting for encoder</code>, so a deliberately slowed run is never mistaken for a freeze; if the encoder ever stalls outright the recording falls back to skipping and tells you. Format, scope, quality and this setting are all fixed at the moment you press Record.</li>
