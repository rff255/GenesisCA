        ...
        <section id="help-nodes" className={styles.section}>
          ...
          <table className={styles.table}>
            ...
            <tbody>
              ...
              <tr><td>Logical Expression</td><td>Type a boolean <strong>formula</strong> in a text field instead of wiring up a chain of Logic nodes &mdash; what <em>Expression</em> is to <em>Math</em>. Operators <code>NOT</code>, <code>AND</code>, <code>XOR</code>, <code>OR</code> (or the symbols <code>!</code>, <code>&amp;&amp;</code>, <code>^</code>, <code>||</code>), parentheses, and the literals <code>true</code>/<code>false</code> (<code>1</code>/<code>0</code> also work). Precedence is <strong>NOT &gt; AND &gt; XOR &gt; OR</strong>, all binary operators are left-associative, and the operator words and literals are case-insensitive. Deliberately <em>no</em> arithmetic and no comparisons &mdash; that is the Expression and Compare nodes&rsquo; job; feed their result in through a port when a rule needs both (e.g. <em>Compare(energy &gt; 5)</em> &rarr; an input named <em>fed</em>, then <em>fed AND NOT crowded</em>). Variables come from the input ports exactly as on the Expression node: add 1&ndash;8 ports, name them, reference the names. The whole editor is the same &mdash; a <strong>rendered logic view</strong> as the node&rsquo;s face (<code>&not;</code> <code>&and;</code> <code>&oplus;</code> <code>&or;</code>, with parentheses re-derived from precedence rather than echoed &mdash; <em>(a AND NOT b) OR (b XOR c)</em> reads as <em>a &and; &not;b &or; b &oplus; c</em>, dropping brackets the precedence already implies, while <em>(a OR b) AND c</em> keeps the pair it genuinely needs), the text field behind a collapsed <strong>&#9656; Edit expression</strong> row, the input names behind <strong>&#9656; Inputs</strong>, and the corner grip to widen the node (double-click it to fit the contents again). An input is <strong>truthy-tested</strong> and the result is a 1/0 binary value, so a formula and the equivalent chain of Logic nodes behave identically. Compiles on all three targets (JS, WASM, WebGPU), on the Cells and Agents graphs alike, and in the Overseer.</td></tr>
        ...
        <section id="help-overseer" className={styles.section}>
          ...
          <ul className={styles.ul}>
            ...
            <li><strong>Log Message / Take Screenshot / Start &amp; Stop Recording /
              ...
              use, which would otherwise stall the sweep waiting for a click.</li>
          </ul>
