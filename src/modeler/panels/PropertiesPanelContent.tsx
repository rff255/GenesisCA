    ...
    <div className={styles.fieldGroup}>
      ...
      <CollapsibleSection id="execution" title="Execution">
        <div className={styles.fieldGroup}>
          ...
          />
          {/* What Reset does. Shown ONLY when the model actually carries an
              embedded BOARD (a "Save with board state" / a captured GIS import):
              with none there is nothing to restore, so the control could do
              nothing and is hidden rather than greyed. It sets the DEFAULT of the
              simulator's ■ button — both actions are always on that button's
              own menu, so nothing becomes unreachable either way. */}
          {hasSavedBoard && (
            <div className={styles.field}>
              <label style={{ display: 'flex', alignItems: 'flex-start', gap: 6, fontSize: '0.72rem', cursor: 'pointer' }}>
                <input
                  type="checkbox"
                  checked={properties.resetRestoresBoard === true}
                  onChange={e => updateProperties({ resetRestoresBoard: e.target.checked })}
                  style={{ marginTop: 2 }}
                />
                <span>
                  <strong>Reset restores saved board</strong>
                  <br />
                  <span style={{ color: '#888', fontSize: '0.66rem' }}>
                    This model carries a saved board. With this on, Reset reseeds from the rules and then
                    applies that board on top — for a board that is DATA (imported map layers, a hand-painted
                    starting configuration) and cannot be regenerated by an Init Event. Off: Reset reseeds only.
                    Either way, the simulator&apos;s ■ Reset button offers both actions on hover / right-click.
                  </span>
                </span>
              </label>
            </div>
