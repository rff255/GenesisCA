...
| B11 | **A Gauss-Seidel (semi-implicit) LINK SOLVER for bond springs.** The `Growing Graphs` physics-parity pass proved the force LAW now matches znah's exactly (0.7–2.5 % on bond length against a same-solver control, and a two-body bonded pair settles at 25.237332 in both to six decimals). The whole residual is the SOLVER: the reference sweeps its edge list twice, forward then backward, on PREDICTED positions (`pos+vel`), which is semi-implicit and therefore stiffer at the same λ — its layout settles 13–25 % tighter (scale-free packing/ring ratios still agree to 3–11 %). It cannot be compensated: raising λ makes our explicit integrator UNSTABLE (measured — λ ≥ 0.7 diverges on a cubic graph at dt/η = 1 and momentum 0.9), and adding Jacobi passes does not change the answer (2/4/8 identical, only the residual force drops). Closing it needs a SEQUENTIAL edge-list sweep, which the per-agent parallel force pass shared by JS/WASM/WebGPU cannot express — the same shape as B10. | Growing Graphs physics parity, 2026-08-04 | `scripts/test-growing-graphs-physics.mjs` reports it every run; CLAUDE.md's Growing Graphs physics table |
| B12 | **A finite cutoff for GLOBAL charge is now SHIPPED** (`chargeMaxDist` under `chargeRange:'global'`), so C10's "global = no cutoff, ever" framing is superseded: the two are now the *same* law with `R = ∞` as the default. The open item is only **guidance** — when a cutoff helps (a growing graph, where the un-cut far field ≈ N/l inflates without bound) versus when it costs quality, and whether an archetype should default one. | Growing Graphs physics parity, 2026-08-04 | CLAUDE.md C10 section; the parity-force cutoff invariant |
