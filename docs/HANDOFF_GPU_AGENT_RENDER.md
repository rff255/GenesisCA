...
| E2 single-canvas composite + 3D lift | READY - handoff written ([HANDOFF_GPU_AGENT_RENDER_E2.md](HANDOFF_GPU_AGENT_RENDER_E2.md); 2D core = one canvas, grid present + agent pass in one encoder, world-space arrangement on the proven blit; 3D voxel pass = an explicitly optional stretch that may STOP) | - | launch next |
