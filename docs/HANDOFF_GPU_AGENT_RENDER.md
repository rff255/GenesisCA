...
7. **Known traps** (all previously hit — do not rediscover them):
   ...
   - The AGENT store arrays are `f64` end-to-end, and `getState.agents.
     ...
     the `stepped` message's `agents`, is f32). Reading a getState f64 buffer
     as f32 fakes ±2^65 "garbage" in ~20% of slots (the A1.5 side-finding was
     exactly this — no bug; the vx/vy save/load is bit-exact). Dump
     `buf.byteLength / hw` (8 ⇒ f64, 4 ⇒ f32) before ever concluding
     agent-velocity corruption.
...
| REGRESSION FIX — WebGPU-agent Chemotaxis (2 symptoms) | **DONE** (session-verified, real live-switch) | `optimize` (this session) | Two distinct root causes on the shipped **Chemotaxis - Aggregation** model, both reproduced via the REAL Properties radios (not pre-set files). **Symptom 1 (WASM grid + WebGPU agent → agents invisible)**: the per-gen/E1b WebGPU-agent path uploads the SoA (`uploadAgentSoA`) but never SEEDED the GPU `agentColors` buffer, and `readbackAgentStep` overwrites `s.colors` with it — so a model whose behaviour has NO colour node (no Set Cell Looks / OM: Chemotaxis + Boids rely on the per-slot DEFAULT_AGENT_COLOR) read back all-ZERO (transparent) colours → invisible on the CPU overlay. FIX = seed `uploadAgentColors(rt, s)` at the end of `uploadAgentSoA` ([agentWebgpuRuntime.ts](../src/simulator/engine/agentWebgpuRuntime.ts)); a colour-writing shader overwrites the seed GPU-side (idempotent), so it only supplies the default when nothing writes. Mirrors the render-path seed (`presentAgentsFromStore`/`uploadAgentRenderFields` already did this). **Symptom 2 (both WebGPU → "agents become a blob of cells")**: agents were CYAN (correct) but the E2 composite renders them at WORLD resolution (`scalePx=1` on a grid-sized canvas → radius-1 disc = ~1px), so once the main-thread blit scales the composite up, each agent is a cell-sized block indistinguishable from the grid cells. This is the E2 world-res tradeoff the report flagged pending eyeball — the user REJECTED it. FIX = disable the E2 composite (`agentComposite = false` in the SimulatorView gate, [SimulatorView.tsx](../src/simulator/SimulatorView.tsx)) so both-WebGPU grid+agents fall back to the DISPLAY-res paths: decoupled → D two-canvas, field-coupled → CPU overlay + grid colors readback (both crisp discs). A DISPLAY-res composite (camera-aware/resampling grid-present shader) is the follow-up REDESIGN the E2 handoff explicitly forbade inventing ad hoc — deferred, NOT a wiring repair. **RENDER/worker-runtime only, ZERO compiler changes** (`git diff --stat` = agentWebgpuRuntime.ts + SimulatorView.tsx). **Real-GPU verified in-browser via the live radio sequence** (load default Chemotaxis → click WebGPU agent → click WebGPU grid): symptom 1 → 220/220 agents CYAN (was 220 transparent), 0 errors; symptom 2 → 220 CYAN + grid colours shipped (`hasColors:true` = readback restored, composite OFF), 0 errors. Regressions clean: Boids-webgpu (resident, no colour node) still 260 cyan; Ant Necrophoresis-webgpu (setCellLooks) shows its real red/grey (seed not clobbered); default WASM+JS Chemotaxis byte-identical (JS-agent path never calls uploadAgentSoA; composite gate was already false for a WASM grid). Gates: tsc + build + parity-agent-wasm (18) + parity-agent-force (7) green; no compiler files → check-compile-identity N/A. Details in the E2 doc's Completion Report. **USER-CONFIRMED (2026-07-23): agents now appear on the WebGPU agent target** (symptom 1 closed in the real app). |
| BUGFIX — agent flow-DIAMOND value scope (all agent targets) | **DONE** (session-verified, real-GPU device-compile) | `optimize` (this session) | User-reported: a Chemotaxis overpopulation rule (a `conditional` on `neighbourDensity >= threshold` whose `then` secretes -0.1 and `else` secretes +0.5, BOTH branches flowing into a SHARED `applyForce` chain = a flow DIAMOND) failed to compile the WebGPU agent behaviour ("unresolved value `_ex17`"). **Root cause**: `compileFlowChain` has NO visited guard (the documented "flow graphs are DAGs" discipline), so a node reached from >1 sibling branch inlines its body ONCE PER branch walk. The shared chain's getRandom-tainted random-walk expression `(a-0.5)*0.05` is NON-hoistable (preEmitAgentValues won't hoist a getRandom cone), so it emitted + cached its `let` inside the `then` block; the `else` walk hit the cache and referenced the then-block's `_ex17` — OUT OF SCOPE (WGSL error), AND the else never re-drew its own random. **Sweep verdicts**: **agentWebgpu** = the reported bug (WGSL unresolved-name). **agentWasm** = SAME CLASS, silently wrong — WASM locals are function-scoped so no validate error, but the else branch reads a stale local (the getRandom `local.set` lives only in the then bytecode) = wrong values + a broken RNG stream (JS↔WASM parity break; proven: the new synthetic FAILS pre-fix, `forceX js=-0.029 wasm=-0.02`). **JS agent** = CORRECT (sink-analysis hoists the shared getRandom to the LCA above the branch; both branches reference it in-scope — no fix). **Fix** (minimal, general — mirrors the lattice sink discipline): a per-flow-block cache-scope snapshot/restore (`enterCacheScope`/`exitCacheScope`) in BOTH agentWebgpu + agentWasm — on entering a `{…}` flow block (conditional then/else, switch cases+default, loop/forEach/forEachBond body) snapshot the value+array caches, on exit drop entries ADDED inside it, so a shared body re-emits its non-hoisted values in each sibling branch's own scope. Snapshot taken BEFORE the entry `clearVolatileCache` (volatile handling unchanged). Pure cross-branch values pre-exist at snapshot time (function-top hoisted) → survive → **byte-identical** for models with no diamond. **Verified**: check-compile-identity 25 models ALL surfaces unchanged; parity-agent-wasm 20/20 incl. a new permanent `[synthetic] Flow diamond` (conditional → shared getRandom chain; proven to FAIL without the WASM fix); parity-agent-force 7; tsc + build. **Real-GPU (device.createShaderModule + getCompilationInfo)**: BOTH the actual user model AND the synthetic device-compile with **0 device errors / 0 validation errors** (the definitive proof string well-formedness can't give); in-app the model loads + runs 5 steps on the WebGPU agent runtime with 0 errors ("falling back to JS" symptom gone); the compiled behaviour retains the density gate + both branch secretes (-0.1 then / +0.5 else). `git diff --stat` = agentWebgpu/compile.ts + agentWasm/compile.ts + parity-agent-wasm.mjs — NO JS-agent/gl3d/engine/lattice/runtime. |
| **AUDIT FIX PASS** | **DONE** (session-verified, pre/post regression proofs) | `optimize` 8441b0a (BLOCKER+HIGH) · 8f35802 (MEDIUM) · 070a1e5 (LOW/NIT) | Closes the read-only audit ([AUDIT_OPTIMIZE_BRANCH.md](AUDIT_OPTIMIZE_BRANCH.md)); work spec + Completion Report in [HANDOFF_AUDIT_FIXES.md](HANDOFF_AUDIT_FIXES.md). **B1 BLOCKER** — `buildAgentWebGPUIfNeeded` now sets `agentGpuUploadPending`; a rebuilt runtime has ZERO-initialised buffers and the resident batch's conditional upload skipped the seed, so a soft recompile dispatched on zeros and the frame readback wrote them into every LIVE CPU slot. **Reproduced pre/post in the real app**: pre-fix all agents → `(0,0)` radius 0, permanent, 0 errors; post-fix preserved + evolving. **H1** the no-bonds gate term applies to BOTH dimensions (the 2D Tissue samples had silently lost their bond lines); **H2** every non-step path that sets `asyncStepBatchInFlight` clears it from a `finally` (a throw dead-locked the worker permanently — reproduced + fixed); **H3** the attach handler reuses a layout-matching render-only surface (6 re-attaches: refCount 1→7 pre-fix, 1→1 post-fix) + L5 view-uniform destroy. **MEDIUM**: M1 the gate's model-dependent terms (sprites / unsupported agent OM) are refreshed on a SOFT recompile via one shared helper + a detach (population survives) — **and the recompile message now carries `agentRenderLayout`, a defect the audit missed that permanently disabled direct render for CPU-target agent models after their first graph edit**; M3 `recompile` joined the one-shot staleness set (a free-mode recompile no longer rewinds); M2 the GPU disc radius is floored at 1.2 px like the CPU overlay; M6 HelpView drift; M5 persistent colour-pack scratch. **LOW/NIT**: L1 backdrop with agents hidden, L3 composite guard in `presentAgentsEncode`, L4 device metrics through `__e1bCounters`, L6/N1/N4 comments. New DEV hook `window.__agentRenderState()` makes the gate matrix probeable. Gates green per commit (tsc, build, parity ×2; check-compile-identity 25 models unchanged for the comment-only compiler edit). Regression sweep: Boids-webgpu 0.998, Chemotaxis WASM+JS unchanged, Chemotaxis WebGPU/WebGPU `gpuBridge 100/cpuFallback 0/refCount 2/adapters 1`, 3D Tissue on the CPU path with bonds. **Left open**: M4 (investigated + planned, no code), L2, N2, N3, and the visible-pane verification debt. |
