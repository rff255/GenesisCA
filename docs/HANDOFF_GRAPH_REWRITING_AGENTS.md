...
| PX WebGPU sync agent attrs | [HANDOFF_GRA_PX_WEBGPU_SYNC_ATTRS.md](HANDOFF_GRA_PX_WEBGPU_SYNC_ATTRS.md) | **DONE** | 2026-07-30 | `59556d2` | The §3.1 design shipped: a second per-attribute run (`agentAttrWriteBase`, **aliased when async** ⇒ every async shader byte-identical) + a per-generation **commit compute pass** (`copyBufferToBuffer` on one buffer is a validation error — the L1 `posCommit` precedent). Reproduced first (**123/56/32/32 of 1024 wrong, varying run to run**; JS/WASM 0) → **0/0/0** after, alive count landing on the exact JS/WASM value; exact over 50/20/100 gens across 3 seeds. **`Life on Bonds` flipped to `webgpu`** and its **O7 differential passes ON THE GPU** (0/1024). `check-compile-identity`: 26 models, exactly 2 diffs, both the SYNC models' `agent.webgpu.shader`. New **Tier E** in `verify-graph-rewrite` (106 checks) with 2 negative controls. **All three §5 assumptions held.** |
| P3 Bond attrs (WebGPU) | [HANDOFF_GRA_P3_BOND_ATTRS_WEBGPU.md](HANDOFF_GRA_P3_BOND_ATTRS_WEBGPU.md) | **DONE** | 2026-07-30 | `c7d2de0` | Bond attributes run on **all three** agent targets. The bondStore slot widened 2 → `2+N` through **ONE** stride constant (`bondSlotStride`, mutation-negative-controlled), binding 11 promoted to `read_write` only when a Set emitter runs (**D3**), Form Bond's initial values ride per-agent request runs, and BOTH gate terms lifted together. **Three things to read**: (a) the **§3 decision is (A) ALLOW + document**, measured — one-sided and symmetric writes are EXACT on the GPU (0 of 32 edges differ, 2D and 3D), only the ASYMMETRIC both-endpoints write diverges (30 of 32; 510 of 512 at N=512) **and tears I2 on 2 edges** — a shape P2's D2 already forbids on every target; (b) **PX's read/write-base pattern was deliberately NOT copied** — the CPU is single-buffered for bonds *by P2 design*, so a GPU write region would have created a NEW GPU-only divergence; true synchronous bond semantics is an **all-three-targets** change, do not schedule the GPU half alone; (c) a **pre-existing JS-only defect** was found and fixed — Form Bond's dynamic INLINE-widget values were dropped on JS (`lbl` read 0 on JS vs 5 on WASM/WebGPU). 200 GPU generations with mid-run bond breaking: I1–I4 green every generation. 26 models byte-identical; `verify-graph-rewrite` 106 → **135** (Tier F) |
| P4 Request queue + Rewire | [HANDOFF_GRA_P4_REQUEST_QUEUE.md](HANDOFF_GRA_P4_REQUEST_QUEUE.md) | **DONE** | 2026-07-30 | `de9cadc` | Bounded per-agent request QUEUE (depth 8, configurable) + the atomic **Rewire Bond** verb, on all three agent targets. **I5**: D+3 ops apply exactly D, the rest rejected WHOLE with a notice, the graph exactly the pre-step graph minus those D edges; a rewire that cannot complete applies NEITHER half. **O5**: N, E and the full degree MULTISET invariant over **500 generations** of a double-edge-swap rule (8000 rewires), I1-I4 green every one. **Multi-op live**: 8 atomic rewires = 16 edge mutations per generation, identical on JS / WASM / WebGPU in the real worker, 0 errors. **26 models byte-identical** (the queue stride is USAGE-gated to 1 for a verb-free graph). `verify-graph-rewrite` 135 -> **180** (Tier G, 5 negative controls); a new permanent parity synthetic with a VALUE invariant, negative-controlled 3 ways incl. a shared-constant mutation parity alone would pass. **Three things to read**: (a) the **`+2` lane bias** is load-bearing - `0`=empty, `1`=side unused, `v+2`=agent - without it a Form Bond with an unresolvable target writes 0 and TRUNCATES the queue, silently dropping every later op; (b) the **overflow BUCKET** (entry D) is why the ABI needed no cursor array and therefore why byte-identity held; (c) the drain now lives in the ENGINE (`drainAgentBondRequests`) so the harness tests the shipped code - P5 should extend THAT, not the worker. |
| P5 Division partition | [HANDOFF_GRA_P5_DIVISION_PARTITION.md](HANDOFF_GRA_P5_DIVISION_PARTITION.md) | **DONE** | 2026-07-30 | `031524e` | `DivideAgent.partition` (**tension** / alternate / byBondAttribute [bool · per-option tag table · int-float threshold]) + **D4** `daughterBond` (auto / always / never), on all three agent targets. **`tension` byte-identical, proven THREE ways**: `check-compile-identity` 26/26; a **HEAD-vs-patched ENGINE A/B in Node over 1000 divisions** (every store field + bond attribute bit-identical, negative-controlled); and a **same-session `git stash` browser A/B** on Growing Tissue (curve, edge count AND summed x-positions identical to 6 dp). **I7 + O9 over 1000 divisions in EVERY mode**, **O4** exact (`N=N₀·2^t`, `E=E₀+N₀·(2^t−1)`) for t=1..8 in every mode, I1–I5 green throughout. `verify-graph-rewrite` 180 → **230** (Tier H), with **five MUTATION negative controls** on shipped source. **Three things to read**: (a) the §2.3 transport is a **per-model TABLE on the EXISTING `divideRequest` cell**, NOT the recommended new lane — a lane sits mid-list in BOTH `AGENT_F64_FIELDS` and `AGENT_GPU_F32_FIELDS` and would diff every agent model's bytes, and the spec (per-option tag vector + threshold + D4) does not fit an f32; (b) unlike `_stopIdx`, the table is **MODEL-derived + key-SORTED and assigned by all three front-ends**, because `parity-agent-wasm` compiles **WASM FIRST** and a first-encounter table would have silently given WASM the wrong code; (c) a **P2 UI gap was closed in passing** — Get/Set Bond Attribute had NO attribute dropdown at all. **`byRule` remains DEFERRED** per §2.4. |
| P6 Graph indicators | [HANDOFF_GRA_P6_GRAPH_INDICATORS.md](HANDOFF_GRA_P6_GRAPH_INDICATORS.md) | **DONE** | 2026-07-30 | `cc524d4` | A third `Indicator.kind`, `'graph'` (node/edge count, mean+max degree, degree histogram, connected components) computed in `sendColors` over the agent store, readable by `ovReadIndicator` with **no Overseer change**. **All three §5 assumptions held.** `verify-graph-rewrite` 230 → **297** (Tier I: 8 fixtures vs INDEPENDENT references — `refComponents` is a BFS flood fill, not a second union-find — a fragmenting graph 1→2→1, the pass counters, 4 data + **7 SOURCE-MUTATION** negative controls). **26 models byte-identical — this phase touched NO compiler.** Real worker: 0 diffs vs a page-side recount from `getState`; O10 through the real panel = mean 121.65 vs theory 112.64, **inside** the 95% CI [110.37, 132.93]; a 16-seed sweep exports **byte-identical CSV across two runs**; Boids (no graph indicator) ran 200 gens with `calls 0, degreePasses 0, componentPasses 0`. `componentCount` costs **2.07 ms at 20 000 agents / 46 247 edges**. **Three things to read**: (a) the §2.4 protocol shipped as a LIBRARY MODEL — `Graph Metrics - Growth Sweep` — so P7 can clone it and swap the rule; (b) **`seedPattern: 'scatter'` uses `Math.random()`**, so a degree-reading rule (degree ← division bond-partition ← tension axis ← positions) is NOT reproducible under it — measured, and fixed by `compact`; (c) **the Overseer seed policy cannot reach the WebGPU agent PCG** (`seedAgentRng` is called once at runtime creation with a hardcoded seed), so sweeps must run on JS/WASM — hence the sample's `agentTarget: 'wasm'`. |
| P4b Form Bond Between | [HANDOFF_GRA_P4B_FORM_BOND_BETWEEN.md](HANDOFF_GRA_P4B_FORM_BOND_BETWEEN.md) | **DONE** | 2026-07-30 | `e1649af` + `6492a51` | `formBondBetween(a, b)` on all three agent targets. **THE GATE MET: the cubic triangle split completes in ONE generation and O6 (`min deg == max deg == 3`, `E == 3N/2`) holds at EVERY generation** — 60 splits in the harness, and **54 splits × 3 targets in the real worker with the WebGPU sequence generation-for-generation IDENTICAL to WASM's**, 0 errors. **All three §5 assumptions HELD** (a newborn IS bondable the same generation: the behaviour pass, incl. the WebGPU newborn reconcile, runs strictly before the structural phase). **Three things to read**: (a) the **§2.2 encoding is the SIGN of the break lane** (`−(a+2)`) — the lanes are `AGENT_I32_FIELDS`, i.e. signed on every target, so the op kind costs **zero new fields and zero moved offsets** ⇒ **27 models byte-identical**; a new lane was rejected for exactly the P5 reason; (b) **the split costs FIVE queue ops** (2 Rewire + 3 Between; Create/Add are host calls consuming no slot) at a TIGHT `maxBonds 3` — the 7-op `2 Form + 2 Break + 3 Between` form also fits depth 8 but needs `maxBonds ≥ 5`, so **P7 needs no raised depth**; (c) a **PRE-EXISTING, phase-BLOCKING defect** was found and fixed in its own commit `e1649af` — `createAgent` was missing from the WASM `AGENT_VALUE_NO_HOIST` (the WebGPU mirror always had it), so the DOCUMENTED spawn idiom *Create Agent → Add To World → consume the handle* failed to compile on WASM and **every behaviour-graph spawning model silently clamped to JS**. `verify-graph-rewrite` 297 → **355** (Tier J), with the decisive negative control that omitting only the `v₂–v₃` edge breaks O6 while leaving I1–I4 green. |
| P7 Samples + docs | [HANDOFF_GRA_P7_SAMPLES.md](HANDOFF_GRA_P7_SAMPLES.md) | **DONE** | 2026-07-30 | `78c294f` | **THE MILESTONE'S HEADLINE, IN A SHIPPED MODEL: `Cubic GRA` holds O6 (`min deg == max deg == 3` and `E == 3N/2`) at EVERY generation** — 220 gens / 847 triangle splits (N 4→1698) plus a 500-gen run, I1–I5 green every generation, the growth law exact (`N=4+2t`, `E=6+3t`), **negative-controlled TWO ways** (drop only the v₂–v₃ edge ⇒ O6 breaks while I1–I4 stay green; disable the priority gate ⇒ adjacent rewriters collide). In the REAL APP at gen 270: N=5120, E=7680=3N/2, deg 3/3, 0 dangling. **O8** in the shipped `SDCA`: 292 links form above λ₂ and **STAY (292→292) back inside the band**, with a single-threshold control that destroys them; verified again live on the WebGPU target (0→0→293→**293**→0). **BOTH MODELS WERE LOOKED AT AND SCREENSHOTTED** (visible pane). `verify-graph-rewrite` 355 → **405** (Tiers K/L load the SHIPPED `.gcaproj`). **27 models byte-identical — this phase touched NO engine, compiler, node or schema file.** **Three things to read**: (a) the **EDGE SWAP IS INEXPRESSIBLE** and that is a real finding, not an omission — a degree-neutral rewiring needs to BREAK an edge between two OTHER agents and no verb does; **`Break Bond Between` is the highest-value follow-up**; (b) the split needs an **independent-set gate** (a random-priority local minimum) — two ADJACENT rewriters is the ONE way it breaks, and the gate is negative-controlled; (c) a **population END CONDITION is part of the O6 contract** — at the agent cap `createAgent` returns −1 and the split half-applies, so the model pauses at N=3000 against a 6000 cap (verified in-app). |
| P8 Motif editor | — | NOT SCHEDULED | — | — | separate decision (D8) |

> **MILESTONE COMPLETE (2026-07-30).** All four Impact Map §1 gaps are closed: **G1** (the
> structural request queue + Rewire + Form Bond Between), **G2** (the declarative division
> bond partition), **G3** (bond attributes on all three agent targets), **G4** (the neighbour
