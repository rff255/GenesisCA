...
| P4b Form Bond Between | [HANDOFF_GRA_P4B_FORM_BOND_BETWEEN.md](HANDOFF_GRA_P4B_FORM_BOND_BETWEEN.md) | READY | - | - | **inserted by the orchestrator while preparing P7** - the triangle split needs an edge between two NEWBORNS, which no self-to-target verb can create, so the flagship oracle O6 is not expressible without it |
