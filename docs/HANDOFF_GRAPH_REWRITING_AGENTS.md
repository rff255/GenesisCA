...
| PX WebGPU sync agent attrs | [HANDOFF_GRA_PX_WEBGPU_SYNC_ATTRS.md](HANDOFF_GRA_PX_WEBGPU_SYNC_ATTRS.md) | READY | — | — | **inserted by P1's finding (b)** — double-buffer the GPU agent attr region for sync models, or reject sync+neighbour-read on WebGPU. Fixes a shipped-model correctness bug; unblocks the WebGPU target for synchronous GRA rules |
