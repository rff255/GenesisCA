...
## PROGRESS — ALL PHASES COMPLETE (2026-07-13)
- ✅ **Phase 0** (`e3a63ea`): schema + Properties UI + ModelContext cascades.
- ✅ **Phase 1** (`cc3a324`): the shared active-set engine (`activeSet.ts`) + the JS sparse step + full worker wiring. Verified sparse==full byte-for-byte (2D+3D) + end-to-end on the real Accretor (correct range == full; too-small range ≠ full ⇒ the gating is real).
- ✅ **Phase 2** (`90eb645`): the WASM sparse step (activeCount param, -1 = full; the list is a wasmMemory VIEW — zero per-step copies) + the sparse colour pass on both targets (post-batch only; every mutation/UI event forces full; glyph models always full). needsFullInit on any config change. Harness runs the REAL instantiated WASM module.
- ✅ **Phase 3** (`04ed54a`): inline neighbours — compact packed-offset tables (size×4) replace the total×size×4 tables when the feature is on; JS decodes via niCellExprStmts, WASM via pushInlineNbrCellIdx at all 6 reachable sites; JS fusion disabled in sparse mode. Verified against a feature-OFF reference compile (three ON arms byte-identical, 2D+3D, JS+WASM) + 300³ inits in 1.9 s (was ~10 s / 2.8 GB).
- ✅ **Phase 4**: the Accretor ships at **300³** with the feature ON (radius-1 chebyshev over `state`=0). Verified: loads in ~1.8 s, steps ~10 gens/s, painting an isolated cell works AND activates it (a painted border cell fired the edge-stop from its own step), ON == OFF grid-hash identity at a fixed seed (40³, JS + WASM). Baseline re-captured (only the Accretor differs; its webgpu/overseer hash changes proven dims-only). Docs updated (CLAUDE.md major section + Accretor bullet, README, HelpView).

**Follow-ups (not blockers):** batch `computeLinkedIndicatorsFromBuffer` to the last step of a step-batch for per-generation linked indicators (saves an O(total) scan per gen — the observer only sees per-batch values anyway); a WebGPU stream-compaction sparse dispatch; `readRegion` is layer-0-only (pre-existing).
