...
**The decisive distinction is memory vs storage.** If "avoid browser resource limits" means the big-grid memory ceiling, **neither** the PWA nor the current Tauri shell helps — both run the simulation in the same Chromium/WebView2 engine (same WASM32 4 GiB cap); only a **native-Rust** simulation would. A native shell *does* escape browser **storage** quotas (real filesystem). If it means "stop evicting my cached models" / "guaranteed offline," PWA + `persist()` is exactly right.

**Tauri v2 (the native follow-up):** wraps the unchanged `dist/` in a Rust host using the OS webview (WebView2/Chromium on Windows). Tauri *can* emit an NSIS `-setup.exe` + macOS `.dmg` + Linux `.deb`/`.rpm`/`.AppImage`, but GenesisCA ships **no installer** — `bundle.active:false`, so the build produces only a standalone portable `genesisca.exe` (the PWA is the main install path; the exe is for offline/USB use). Its `fs` plugin gives true native file read/write **bypassing browser storage quotas entirely** — but the simulation still runs in the host webview (same WASM/JS engine → **same memory ceiling as the PWA**), so the native win is storage + filesystem + a clean window, **not** compute. ~3–10 MB bundle vs Electron's ~85 MB. **Prototype-first caveat for THIS app:** Tauri uses the host webview, so on **macOS/Linux it's WebKit, where WebGPU + some Canvas/WASM behaviors differ** — GenesisCA's WebGPU compile target may need to fall back to WASM/JS on those targets. On Windows (WebView2 = Chromium) parity with the PWA is high.

**One-line recommendation:** ship the PWA now (installable Windows app + own icon + cross-platform install + full offline, today, near-zero risk, reuses `dist/`); plan Tauri v2 as a follow-up if the real constraint is native filesystem / storage quota or a clean toggle-free native window (**not** memory — the wrapper shares the PWA's engine; only native-Rust compute would lift that) — same `dist/`, native fs, no quota — and prototype WebGPU-on-WebKit before committing to non-Windows native builds.

---

## (D) Open decisions

1. **Which "resource limit" do you mean?** Disk/cache durability (PWA solves it) vs RAM for large grids (NOT solved by the Tauri wrapper — same engine; needs native-Rust compute). Determines whether Tauri is "later" or "soon."
