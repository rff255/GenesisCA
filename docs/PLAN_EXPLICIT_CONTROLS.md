...
## 13. Deviations found during P3 — 2026-08-26

*Recorded, never silent (the §1 discipline). Everything not listed here followed
§6 as written. P3's gates: `tsc` · `npm run build` (both builds) ·
`check-compile-identity --compare` **31 models, all surfaces unchanged** ·
`test-explicit-controls` **258 checks** (197 → 258: tier G's 61) with **seven**
source mutations proven caught · `test-macro-references` ·
`test-param-input-mappings` · `check-agent-wasm-gate`.*

| # | Plan | What shipped | Why |
|---|---|---|---|
| **P3.1** | §6 — the badge is "one new roll-up line … when any control resolves with a `block`" | **`CONTROL_BLOCK_NEEDS_ATTENTION`** — the BROKEN blocks only (`orphan-*`, `cycle`). `wired` and `scope-open` render disabled with their reason on the ROW and do **not** badge | A badge is the "something is wrong, look inside" signal, and both excluded states are deliberate and healthy: a wired parameter is how macros are normally built, and `scope-open` lasts exactly as long as the macro is open. Badging them fires the alarm on a correct model — the noise the enabled-control doctrine exists to prevent. It is a NAMED, exported set (not an inline test), so the badge, the harness and any later consumer cannot disagree; tiers G17/G31/G35/G40 pin both halves |
| **P3.2** | §6 tier G — "`REMOVE_ATTRIBUTE` … the control **still resolves**, now reading `''`" | Two outcomes, both correct: an **unrelated** control still resolves live (G21 — the F4 claim), while a control bound to the `setAttribute._port_value` of the DELETED attribute reports **`orphan-key`** (G22), still present (G23) | That value port's widget is ADAPTIVE: with no attribute the node renders **no value widget at all**, so the parameter genuinely no longer exists. Resolving it to a bare number field would invite the user to type a number into a slot that will be re-typed the moment the attribute is re-picked — plausible-but-wrong, silently. `orphan-key`'s sentence points at the real problem, and the internal node's own `detectMissingConfig` badge names the dangling id |
| **P3.3** | §6 — "ungrouped rows first, then each group in `def.groups` order under a `.sect`-style header" (no helper named) | **`groupSections(items, groups)`** in `explicitControls.ts`, built **by CALLING `orderByGroup` and partitioning its output into consecutive runs** | P2 left the instruction that the instance must render "with the SAME call the editor reorders with". Re-deriving the order here would be a second definition free to drift; calling `orderByGroup` and slicing its output makes "same order" true by construction (the partition is exact because that function emits the ungrouped head first and each group CONTIGUOUSLY). Tier G2 asserts the flattened sections ARE `orderByGroup`'s output, and the "re-derive the order" mutation is caught |
| **P3.4** | §6 — "CaNode reads the open scope from a graphState mirror of `currentScopeRef` (a second tiny global in the same shape)" | That, **holding DEF IDS ONLY** — `setOpenMacroScope` filters the editor's leading `'root'` sentinel — set in the scope-switch effect and cleared in the Esc effect's unmount cleanup (the two sites P2.5 established) | `'root'` names no def and could never match one, so filtering it once at the setter keeps every consumer a plain list. The two sites are the same argument P2.5 made for pick mode: the scope effect's deps carry `modelVersion`, so ONE call covers a scope change, a graph swap and a model load, and GraphEditor is unmounted whenever the user is not in the Modeler |
| **P3.5** | §6 real-UI step 6 — "Collapse the instance → controls vanish; expand → they return" | **Not reachable in the UI**: a macro instance's double-click ENTERS the macro (`onNodeDoubleClick`, GraphEditor L3976-3990) and its context menu offers no Collapse. Pinned STRUCTURALLY instead (G60: `if (!showExpanded)` returns before the body div, hence before the section) | Nothing in P3 changed this — a macro instance has never been collapsible. The substantive half of step 6, the handle-offset measurement, WAS done and is stronger than the recipe asked for (below). The badge roll-up is what would surface a broken control on a collapsed node, which is why P3.1 keeps it |
| **P3.6** | — | A `select` row renders a synthetic trailing `<option>` for a value the LIVE option list no longer contains (`(unset)` for `''`, else `<value> (?)`) | Without it a stale stored value renders as the FIRST option — the widget would silently claim a value the config does not hold, and the next edit would write that lie back. The same reasoning as the tag-name-not-index rule elsewhere |
| **P3.7** | — | **`window.__getOpenMacroScope`** added beside `__setControlPick` (`import.meta.env.DEV`-gated) | The R7 mirror decides whether a closed instance is writable and is otherwise unobservable from a probe. It is what proved the wiring live (`[]` at root → `['macro_mno12xxa']` inside the macro → `[]` on exit) |

### Verification notes worth keeping

- **⚠ A SOURCE PIN WRITTEN AS `.includes('…')` ALSO MATCHES THE LINE COMMENTED OUT.** The `setOpenMacroScope(currentScope)` mutation (comment the line) was the ONE of seven that slipped through on the first pass — the check passed on `// setOpenMacroScope(currentScope);`. Every "this statement exists" pin must require a real STATEMENT: `/(^|\n)[ \t]*setOpenMacroScope\(currentScope\);/`. Re-run your mutations after fixing a pin — this one was only found because the mutation table had a row that reported **0 failures**.
- **THE HANDLE-OFFSET PROOF (F6), measured three times on the same instance** as controls were added: node height **95 → 180 → 223 px**, handle `top` offsets **49 / 71 at every one**. (The handles' `left` follows the node's own width — 150 → 154.78 → 182.77 — because a CaNode is content-sized and a widget widened the body; that is true of every existing body widget and is not a handle move.) A PORT group reorder then SWAPPED the two handles (`out_1` to top 49, `out_0` to 71) through the pre-existing `portIdSignature` remeasure effect — no new `updateNodeInternals`.
- **THE EDGE-IDENTITY CLAIM WAS PROVEN BEHAVIOURALLY, not from the DOM** (an occluded pane renders zero React Flow edges — P2's finding). After grouping Game of Life's `Must Die` port so the exposed-output array reordered, a blinker stamped through the real worker oscillated **cell-for-cell identically to P2's pristine sequence** (`[[99,100],[100,100],[101,100]] → [[100,99],[100,100],[100,101]] → …`, period 2 over 3 steps). An inverted bridge would have destroyed the rule immediately.
- **⚠ THE UNDO HISTORY IS CLEARED ON EVERY SCOPE SWITCH** (`clearHistory()` in the scope effect), so a delete-inside-the-macro and its Ctrl+Z must happen in ONE visit. Verified there: deleting the target node flipped the editor's subtitle to *"its target node was deleted inside the macro"* and Ctrl+Z restored *"Count Matching · Operation"*. The control that was orphaned across a scope switch stayed orphaned **and stayed present** — which is exactly the argument against auto-delete.
- **⚠ A MACRO NODE'S DOUBLE-CLICK ENTERS THE MACRO.** Two probes were written expecting it to toggle collapse and threw on a null node afterwards (both logged as `onerror` — probe bugs, not app errors). The same trap explains P3.5.
- **Real-UI coverage, all with 0 fresh console errors**: five controls bound from BOTH an in-place widget and an overlay row; the closed instance renders `PARAMETERS` → the two ungrouped rows → the `Tuning` header → its row, i.e. `orderByGroup`'s order; **instance → macro** (3 / true / equals → 5 / false / greater, read back off the internal widgets) and **macro → instance** (7) both ways; **two LINKED instances** both showing `⛓ 2` and moving together from one edit (11 → 11), then **Duplicate Independent** diverging (99 vs 11) with no badge — R2 in the UI; a **WIRED** control read-only at `0` with *"wired inside the macro"* and no badge; a deleted target greying to `—` with the danger-coloured reason **and** the instance badge reading exactly *"1 control needs attention"* (the wired one deliberately not counted); a fresh model rendering **no section at all** (back to exactly 95×150). Widget shapes exercised live: **number**, **bool select**, **enum select**, **checkbox** (two controls on ONE key flipped together from a single click — D1 at its sharpest), **textarea** (stacked, 100% width, `J + is_south*G` → the internal node's rendered math becoming `J+is_south·G·2`), and the read-only blocked form. `glyph` / `text` / `color` / class-A `tag` are covered functionally by tiers A and G but were not rendered in a browser — no shipped macro contains one.
- **Never `git checkout <file>` to revert a source mutation on an unstaged file.** Every mutation was applied by a line-based, CRLF-agnostic helper and reverted from a copy in the scratchpad, with a `cmp` proving each restore byte-exact.

### The seven source mutations, and what each broke

| mutation | caught by |
|---|---|
| the instance FILTERS blocked rows out of `controlSections` (auto-drop) | G50b |
| `resolveControlDescriptor`'s `blocked()` omits `block` (a broken control reports as LIVE) | 13 checks across A and G |
| the `built.wired` branch is dropped (a wired port resolves live) | A18, E11, G32, G34 |
| the write handler reads the captured `model` instead of `modelRef.current` | G52 |
| `groupSections` re-derives the order instead of calling `orderByGroup` | G1, G2 |
| `CONTROL_BLOCK_NEEDS_ATTENTION` includes `'wired'` | G35 |
| the editor stops mirroring the open scope (R7 dies silently) | G58 |

### What P4 inherits

- The instance is now a live surface, so class C only has to make `elementOptionsFor` return a list — `describeKey`'s class-C arm, `eligibleControlKeys`' `classes` gate and the `'element'` widget (a plain `<select>` over `descriptor.options`, including P3.6's stale-value option) already exist and are exercised by every other select.
