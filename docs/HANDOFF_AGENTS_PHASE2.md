...
2. **Numeric boundary case** (the proof) via `window.__simWorker` on Boids (120×120 torus): place agent P at `(1, 60)`, neighbour Q at `(119, 60)` (true offset +2 across the seam). Get Agent Offset (P→Q) must emit `dX ≈ +2, dY = 0, Distance ≈ 2` — NOT `dX ≈ −118`. Cohesion on P (only Q nearby): `sumX/n = +2`, force-x `> 0` (steers +x toward the seam). Separation on P: `−dX/(d²+1) = −2/5 < 0` (pushes −x away). Q→P: `dX ≈ −2`.
