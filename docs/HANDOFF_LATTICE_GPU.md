...
| L2 pipelined step batches | **SHELVED** — measure-first gate FAILED (2026-07-25, visible pane) | — | The premise (a flat, grid-size-INDEPENDENT ~7 ms submit→fence latency dominating each batch) does NOT hold. Measured Coagulation on the WebGPU grid, visible pane, g/s-vs-gens/frame decomposition: at **300²** the per-batch cost is ~29 ms and ~constant across gf1/8/64 (fixed-overhead-bound) but is TRIVIALLY amortized by raising gens/frame (gf64 → 1792 g/s); at **2000² (4M cells)** a 2-point fit (gf1=333 ms, gf64=500 ms per batch) gives **fixed overhead ≈330 ms + kernel ≈2.65 ms/gen**. The fixed overhead SCALES with grid (29→330 ms for 300²→2000²), so it is NOT the constant submit→fence latency L2 targets — it is cell-count work (the main-thread display blit of a 2000² canvas and/or a per-batch readback), which L2's GPU-batch pipelining does not touch. So the flat latency is both small and not the bottleneck. **Redirect** if large-grid throughput matters: the main-thread display blit (an offscreen/direct-present optimization) or GPU indicator reductions (L6), NOT L2. Caveat: these are machine-specific end-to-end play numbers; a worker-internal batch-timing probe could refine the attribution but the direction is clear. **CORRECTIONS (2026-07-27):** (a) the "330 ms main-thread blit" attribution above was measured on the since-REVERTED display-res-present tip and was NEVER verified on the base — a direct base measurement put the 1000² `drawImage` scale at ~0.002 ms, so do NOT cite that number as a base bottleneck (the display-res grid-only present built on it was withdrawn, `b2e544b`); (b) a later in-worker decomposition (see [HANDOFF_LATTICE_GPU_L2.md](HANDOFF_LATTICE_GPU_L2.md)'s premise + the withdrawal record in [HANDOFF_LATTICE_DISPLAY_RES_PRESENT.md](HANDOFF_LATTICE_DISPLAY_RES_PRESENT.md)) found the per-batch fixed cost at 1000² is the PRESENT/FENCE vsync coupling (~2 display frames) — L2-shaped after all — but TWO decoupling attempts (present-after-fence; one-deep pipelined fence) measured no improvement and the test box drifted ±40% between same-condition runs, so L2 STAYS SHELVED pending a stable interleaved-A/B rig and a demonstrated win on the user's machine (whose base overhead is already ~5 ms/batch). See [HANDOFF_LATTICE_GPU_L2.md](HANDOFF_LATTICE_GPU_L2.md). |
...
| L4 inline neighbours (CPU) | **SHELVED** — value analysis (2026-07-25) | — | The only benefit is dropping the CPU per-cell neighbour table on the JS/WASM targets (memory). But that table is only large at big 3D grids, which are impractical on CPU targets anyway (a 300³ JS/WASM step is ~seconds/gen), and the WebGPU target — the one that runs those grids — ALREADY drops the table (`3c8f03d`) + the write buffer (`b385a34`). On CPU, inline neighbour computation ADDS ALU to the hot per-cell loop vs a cache-friendly precomputed-table read, so it likely SLOWS the common case to save memory at a scale CPU can't reach. Low value, real regression risk on the CPU step. Reopen only if a concrete need arises (e.g. a mid-size 3D grid that must run on WASM under a memory-constrained tab). design in PLAN §L4. |
