...
**Scope:** 154 registry node types across 7 categories (event, flow, data, logic, aggregation,
output, color) — 3 hidden from the Add Node menu (`macro` / `macroInput` / `macroOutput`),
leaving **151 selectable** (52 of them agent nodes).
...
AND agents) — and **Periodic Step** [`periodicStep`], an agent event root whose chain runs only
every Nth generation. Cadence is *model semantics*, so it lives in the rule graph rather than
as an engine knob; Periodic Step is lowered to `Get Generation → Math(%) → Compare → If/Then`
