...
| 27 | `getRandom` | Get Random | Random binary / integer / decimal (uniform, normal or exponential) / orientation, one option picked uniformly from a wired Options array, or a random **vector** of a given length and angular spread. | `I: P` (float, Binary), `I: Min` `I: Max` (float, inline, Integer + uniform Decimal), `I: Mean` `I: Std Dev` (float, inline, normal/exponential Decimal), `I: Options` (any, isArray) `I: Fallback` (any, inline) (Options), `I: Norm` `I: Angle°` `I: Dir X` `I: Dir Y` `I: Span°` (float, inline, Vector) / `O: Value`; Vector mode instead exposes `O: X` `O: Y` (float) + `O: Vector` (vector) | **Every parameter is a PORT**, so Min/Max/Mean/σ/Norm/Angle/Span can be driven by a wire. Draw counts (identical on every target): normal = **2** stream advances (Box-Muller, fixed count — never a rejection loop), everything else = **1**. Vector: the direction is uniform within ±Span°/2 of a reference — a compass **Angle°** (0° = north = −y, 90° = east = +x, matching the sprite heading convention) or a wired **Dir X/Y**; a zero wired direction falls back to north. Planar (XY) in 3D models too. The composite `Vector` output lowers (via `expandComposites`) to this node's own X/Y, so wiring it costs no extra draw. Legacy `config.min`/`config.max` are migrated onto the ports on load |
