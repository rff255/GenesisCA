...
| 80 | `breakBond` | Break Bond | `output` | Request that the bond between this agent and a target be removed (symmetric) — applied post-step. | `I: DO` `I: Target` (int) / `O: NEXT` (flow) | Typically fed by `forEachBond`'s `Partner` (e.g. break over-strained bonds). **P4: requests are QUEUED** — an agent may break several bonds in one step |
