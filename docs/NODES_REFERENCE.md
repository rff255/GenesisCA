...
| 124 | `transferBond` | Transfer Bond | `output` | **Hand this agent's bond with `Partner` over to `New Partner`, rewriting Partner's slot IN PLACE.** The order-preserving sibling of Rewire Bond, which is break+form at the REQUESTER and therefore compacts (scrambles) the third party's list. Slot order decides which neighbour each daughter of a split inherits, so it propagates into the structure — and the embedding — forever. The bond KEEPS its rest length, stiffness and every bond attribute: it is the SAME edge re-pointed, not a fresh one, which is also what makes I2 hold by construction (the mirror slot appended at the new partner is stamped with those same values). | `I: DO` `I: Partner` (int) `I: New Partner` (int) / `O: NEXT` (flow) | Applied in the post-step structural phase via the REQUESTING agent's own request QUEUE (the two ids are PAYLOAD, not addresses — **WebGPU still needs no atomics**). **ALL-OR-NOTHING** (invariant I5): no-op unless this agent is actually bonded to Partner, New Partner is live / in range / distinct from both, Partner is NOT already bonded to New Partner (that would be a double edge), and New Partner has a free slot. Degrees after: Partner unchanged, this agent −1, New Partner +1. Capability: Bonds. **ALL THREE agent targets** (JS↔WASM bit-parity) |
