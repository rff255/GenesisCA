...
| 115 | `setAgentSprite` | Set Agent Sprite | `color` | Control the agent's sprite exhibition: independently-tickable facets — **Change sprite** (`spriteId`), **Set frame** (the `Frame` input — jump/reset), **Set speed** (the `Speed` input — frames per step, **negative = reverse**, 0 = hold), **Set rotation** (an angle in compass degrees OR a Dir X/Y vector the art aligns to), **Set scale** (per-agent size multiplier), **Set alpha** (the agent colour's alpha byte 0–255 — the sprite blit multiplies by it; a later colour pass overrides it). Tick only what you want to change. The per-agent state is persistent; the engine advances `frame += speed` each step; the render floors + wraps (loop) / clamps (once). | `I: DO` `I: Agent` (optional target id) `I: Frame` (int) `I: Speed` (float) `I: Rotation°`/`I: Dir X`/`I: Dir Y` `I: Scale` `I: Alpha` (each shown when its facet is ticked) / `O: NEXT` (flow) | Writes the per-agent display buffers `spriteIds`/`spriteFrames`/`spriteSpeeds`/`spriteRotations`/`spriteScales` (+ the agent colour's A byte). Those live in the SHARED agent memory (the sprite block in `computeAgentMemoryLayout`), so this node **emits on JS and WASM**. **WebGPU is the exception** — no GPU mirror, so a behaviour-reachable Set Agent Sprite clamps a WebGPU-target model to JS (Compatibility names it); in an Agent **Output Mapping** graph it is all-target-clean (OM passes are JS on every target). Manage sprites in the Mappings panel → Sprites |
