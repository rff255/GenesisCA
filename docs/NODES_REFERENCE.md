...
**Scope:** 149 registry node types across 7 categories (event, flow, data, logic, aggregation,
output, color) — 3 hidden from the Add Node menu (`macro` / `macroInput` / `macroOutput`),
leaving **146 selectable** (**Get / Set Bond Attribute** [`getBondAttribute` /
...
appear in the editor only when their respective model feature is enabled (the 50 agent
