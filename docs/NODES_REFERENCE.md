...
leaving **150 selectable** (53 of them agent nodes).
...
| 54 | `setAttribute` | Set Attribute | Write a value to an attribute of the current CELL — or, on the AGENTS graph, of an agent: the optional `Agent` input is the standard targeting convention (**empty = the current agent**, wired = that agent by id, e.g. signalling a neighbour or configuring a Create Agent handle). Extra attribute slots write several attributes in slot order, each with its own `value_N` input + type-adaptive inline widget; a wired `Agent` FANS OUT to every slot. | `I: DO` (flow) `I: Agent` (int, Agents graph only) `I: Value` + `I: value_N` per extra slot / `O: NEXT` (flow) | Multi-slot expands to a linear single-set flow splice pre-compile. The `Agent` port is hidden on the Cells graph (`hiddenPorts`, graph-aware). **A WIRED id is a cross-agent OVERWRITE**: rejected in SYNCHRONOUS agent mode and by the WebGPU agent gate unless it comes from a Create Agent handle — such a model runs on JS/WASM. Absorbed the retired `setAgentAttribute` ("Set Attribute (by ID)"); legacy files are migrated on load |
...
| 101 | `setAgentsAttribute` | Set Agents Attribute | `output` | Write one attribute on EVERY agent in an id array — the write-many companion to a by-id Set Attribute. Feed it Get Nearby / Bonded / Filter Agents to signal a whole group. | `I: DO` `I: Agents` (int **array**) `I: Value` (float, inline) / `O: NEXT` (flow) | Requires `attributeId`. Immediate (async-style) writes, each id range + alive guarded |
| 102 | `setVelocity` | Set Velocity | `output` | Set an agent's velocity directly — the momentum companion to Apply Force (seeds the integration velocity rather than accumulating a force). **Agent empty = self, else that agent by id** (mirrors Get Velocity) — e.g. a knock-back. | `I: DO` `I: Agent` (integer, optional) `I: Vx` `I: Vy` (float, inline); 3D adds `I: Vz` / `O: NEXT` (flow) | Only meaningful when the model's Momentum > 0 (overdamped mode recomputes velocity from the force each step — use Apply Force there). Writes `_agentVX/VY[id]`. A WIRED id is a cross-agent OVERWRITE ⇒ rejected in Synchronous agent mode + by the WebGPU agent gate (non-spawn); use Apply Force To Agent for an order-independent alternative. `Vz` hidden in 2D (and dropped in the Init Event — no `_agentVZ` param there) |
...
| 108 | `getSelfHandle` | Get Self Handle | `data` | The CURRENT agent's own handle (its id = the loop `idx`) — pass it to the by-id nodes (Get Agent Attribute, a wired Set Attribute, Get Agent Position/Offset/Radius, Form/Break Bond) so a neighbour can reference back to me, or to compare a Get Nearby Agents id against self. | `O: Handle` (int) | Emits `idx`. Runs on all three agent targets (JS / WASM / WebGPU) |
