...
attribute. JS + WASM (bit-parity); the WebGPU agent target REJECTS a model declaring bond
attributes and clamps to WASM/JS with a stated reason [P3 lifts it];
...
| 121 | `getBondAttribute` | Get Bond Attribute | `data` | Read a BOND's user attribute by PARTNER ID — per-EDGE state (the value belongs to the edge, not to either endpoint). Feed `Partner` from For Each Bond / Get Bonded Agents. Bonds are symmetric, so either endpoint reads the same value. | `I: Partner` (int) / `O: Value` (any) | Requires `attributeId`. No bond with that partner ⇒ the attribute's **default** (never NaN). Per-agent (never hoisted) + impure (a bond attribute is mutable mid-step, and compaction moves slots). Capability: Bonds. **JS + WASM** (bit-parity); the WebGPU agent gate rejects a bond-attribute model (P3) |
| 122 | `setBondAttribute` | Set Bond Attribute | `output` | Write a BOND's user attribute by partner id. **Writes BOTH endpoints' slots** — a bond is one object stored twice, and invariant I2 requires them to agree, so there is no "my side". A directed quantity is expressed by STORING an owner/direction value (e.g. an `ownerId` bond attribute compared against Get Self Handle), never by writing the two sides differently. | `I: DO` `I: Partner` (int) `I: Value` (float, inline) / `O: NEXT` (flow) | Requires `attributeId`. No such bond ⇒ no-op; the partner side is range + alive guarded. A bond formed THIS step doesn't exist yet (Form Bond applies in the structural phase) — seed a new bond's values on Form Bond itself. Capability: Bonds |
