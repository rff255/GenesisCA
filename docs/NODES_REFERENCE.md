...
| 125 | `logicalExpression` | Logical Expression | Type a boolean formula instead of wiring a chain of Compare + Logic nodes — what Expression is to Math. Operators `NOT AND XOR OR` (or `! && ^ \|\|`), the comparisons `< <= > >= == !=` (`=` is `==`), parentheses, numbers, literals `true`/`false` (or `1`/`0`). Precedence `CMP > NOT > AND > XOR > OR`, so `NOT n > 2` is `NOT (n > 2)`; boolean binary operators are left-associative, comparisons do NOT chain (`a < b < c` is an error naming the `AND` rewrite); operator words and literals are case-insensitive. A variable is dual-role — bare it is truthy-tested, in a comparison it is its raw number. No arithmetic (not even negation): compute in an Expression node and wire it in. | dynamic `I: a…h` (**`any`** — the same operand typing Compare uses, so numeric sources are offered; 1–8 ports, configurable count, each renamable — same collapsed-by-default `▸ Inputs` row as Expression; the inline widget stays bool) / `O: Result` (bool) | Parses to its OWN small AST (the boundary with Expression's grammar is ARITHMETIC and only arithmetic — this one has no arithmetic, that one has no boolean operators); JS / WASM / WebGPU lockstep on both the cell and agent loops, plus the Overseer driver. The node body renders that AST with `¬ ∧ ⊕ ∨` + `< ≤ > ≥ = ≠` and parentheses re-derived from precedence. Boolean reads are truthy-tested (`≠ 0`) and the result is the project's numeric bool 1/0 — identical to `logicOperator`; comparisons reproduce `statement` (Compare) exactly, f64 on the CPU targets and f32 on the GPU ones, with exact `==` / `!=` |
