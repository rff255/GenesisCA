...
| 79 | `formBond` | Form Bond | `output` | Request a (symmetric) bond between this agent and a target agent — applied in the post-step structural phase. No-op if already bonded, the target is dead, or either list is full. | `I: DO` `I: Target` (int) `I: Rest Length` `I: Stiffness` (float, inline) + **one input per BOND ATTRIBUTE** (labelled with its name, type-adaptive inline widget) / `O: NEXT` (flow) | NOT async-only. Rest Length 0 = contact distance; Stiffness 0 = the model's λ. Auto-bond forms bonds by distance with no node. The bond-attribute inputs seed the new bond's INITIAL values (both slots — I2); absent ⇒ each attribute's default, and a model with no bond attributes emits byte-identical code. **P4: requests are QUEUED** — an agent may form SEVERAL bonds in one step (depth in Properties → Bond-Graph Agents; ops past it are rejected whole with a notice) |
