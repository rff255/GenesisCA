...
| 88 | `getAgentAttribute` | Get Agent Attribute | `data` | Read a specific agent's attribute by id (the agent analogue of Get Neighbor Attribute By Index) — differential adhesion, contact inhibition, signalling. Extra attribute slots read several of that agent's attributes through the ONE shared Agent input (fanned out at expansion). | `I: Agent` (int, REQUIRED) / `O: Value` (any) + `O: value_N` per extra slot | Requires `attributeId` (+ each slot's `attr_N`) AND a wired `Agent` — an unwired id is the `-1` empty sentinel, which the range guard resolves to a SILENT read of **0** on every target, so `detectMissingConfig` badges it and names Get Self Attribute instead (same badge on `getAgentPosition` / `getAgentRadius` / `getAgentOffset` / `setAgentPosition` / `setAgentRadius`). CSE-impure (a neighbour write can mutate it) |
...
| 105 | `setAgentPosition` | Set Agent Position | `output` | Set an agent's position by id — a spawn helper for a staged Create Agent handle (also works on a live agent). | `I: DO` `I: Agent` (int) `I: X` `I: Y` (float, inline); 3D adds `I: Z` / `O: NEXT` (flow) | In the Init Event the guard is range-only (a staged agent is `alive=0`); elsewhere it requires a live agent. `Z` hidden in 2D |
| 106 | `setAgentRadius` | Set Agent Radius | `output` | Set an agent's radius (and growth target) by id — a spawn helper for a staged Create Agent handle (also works on a live agent). | `I: DO` `I: Agent` (int) `I: Radius` (float, inline) / `O: NEXT` (flow) | Writes both `_agentRadius` and `_agentTargetRadius` so the growth ramp doesn't drag it away |
