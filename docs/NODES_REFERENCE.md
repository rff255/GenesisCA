...
**Scope:** 156 registry node types across 7 categories (event, flow, data, logic, aggregation,
output, color) — 3 hidden from the Add Node menu (`macro` / `macroInput` / `macroOutput`),
leaving **153 selectable** (53 of them agent nodes).

**The newest nodes are the two GLOBAL PERIODIC ROOTS** — **Grid Periodic Event**
[`gridPeriodic`] on the Cells graph and **Population Periodic Event** [`agentPeriodic`] on the
Agents graph. Each runs its chain **ONCE GLOBALLY** (not per cell, not per agent, no `self`) on
generations where `generation % Period === Phase` — the periodic counterpart to the Grid Init
Event and the Agent Init Event, for adding substrate, spawning a wave of agents, sweeping a
population, or reading an indicator and firing a Stop Event. They land at the TOP of the
generation, before the agent step and before the cell step. Both compile to ONE JS function the
worker executes on EVERY compile target (the Grid Init Event architecture), so no WASM/WebGPU
emit and no capability gate changed. **The pre-existing `periodicStep` was RENAMED to
`Agent Periodic Step`** in the same pass — it keeps its type id, but its old unqualified name
read as global when it is emphatically PER AGENT, which is the confusion these roots resolve.

The change before them was not a node but a BRUSH KIND: an AGENT Colour→Attribute mapping declares
`brushKind: 'editor' | 'spawner'`** (absent ⇒ `editor`, the historical behaviour, no migration).
An **Editor** brush runs the graph once PER AGENT the footprint covers, with that agent as `self`
(and may now also spawn agents around them / remove them — the `input` ABI carries the grow-only
`_agentCreate` / `_agentAddToWorld` closures + the `_killRequest` lane, drained IMMEDIATELY by the
paint handler). A **Spawner** brush runs it ONCE PER BRUSH APPLICATION with NO `self`: its root
gains **`Brush X` / `Brush Y` / `Brush Radius`** value outputs (plus `Brush Z` in 3D, appended to
the trailing 3D block so 2D stays a strict prefix) and it creates the agents itself. The two shapes
are the `input` and `spawner` kinds of the shared agent ABI descriptor; `spawner` is structurally
the Agent Init Event's signature plus the brush block. Kind is resolved ONLY through
`inputBrushKindOf` (`src/model/inputMappingParams.ts`), and a spawner→editor flip DESTROYS the
brush ports, so their wires are dropped by the same drop-never-repoint cascade a deleted parameter
follows.

**The change before it was also a port shape: the two input-mapping roots (`inputColor`,
`agentInputMapping`) have DYNAMIC value outputs** — one port per PARAMETER its Colour→Attribute
...
The newest CHANGE adds **no node at all**: the **Division Event** grew a `Sibling` output (the
OTHER daughter's id) and the four bond verbs became valid inside it, so a daughter can be
configured by id and BONDED TO A THIRD PARTY in the generation it is born — the one edge the
agent tier could not express (the partition only redistributes the mother's existing bonds, and
the A–B policy only adds the daughter–daughter edge). Both ride the `division` ABI as **gated,
trailing** blocks and the requests are applied by a SECOND structural drain, so a model that uses
neither is byte-identical. The newest NODE is still **Agent Input Mapping (C→A)**
...
WASM/WebGPU emit. The pair before it was **Transfer Bond**
[`transferBond`] — hand this agent's bond with one partner over to a new partner, rewriting the
PARTNER's bond slot **IN PLACE**. Rewire Bond moves the same edge but is break+form at the
REQUESTER, and a break compacts by swapping the list's last entry into the freed slot, so it
SCRAMBLES the third party's adjacency order; Transfer overwrites the slot where it stands. Slot
order is not cosmetic for a graph automaton — a cubic triangle split keeps slot 0 and hands
slots 1 and 2 to its daughters — so it propagates into every later split. The bond KEEPS its
rest length, stiffness and attributes (it is the same edge re-pointed), which is why the node has
no value ports beyond the two ids. Encoding-wise it is the mirror image of the *between* encoding
(a paired Form Bond): the op kind rides the sign of the FORM lane instead of the break lane, so it
costs no new field and moves no baked offset. The previous pair was the **rule cadence**
one: **Get Generation** [`getGeneration`] — the current generation number, UNIVERSAL (cells
AND agents) — and **Agent Periodic Step** [`periodicStep`], an agent event root whose chain runs, FOR
EVERY AGENT, only every Nth generation. Cadence is *model semantics*, so it lives in the rule
graph rather than as an engine knob; Agent Periodic Step is lowered to
`Get Generation → Math(%) → Compare → If/Then`
before any target compiles, so it costs zero per-target emit. The **Graph-Rewriting Automata** milestone is COMPLETE at these
...
vocabulary. (**Form Bond** names BOTH ends: its optional `Agent A` port defaults to
THIS agent when unwired, and wiring it lowers the op to the *between* encoding — so ONE node covers
both "bond me to X" and "bond X to Y". A separate **Form Bond Between** node shipped that encoding
first and was **RETIRED** into this port once it became redundant; a legacy `.gcaproj` is rewritten
on load by `src/model/formBondBetweenMigration.ts` (`agentB` → `Target`) and emits byte-identical
code. The two-id form exists because the cubic **triangle split** needs the edge joining two agents
the rule CREATED THIS GENERATION: neither is `self`, and neither runs its own behaviour until the
next generation, so without it the split has to span two generations and the intermediate state
violates the very invariant the rule preserves. The request rides the REQUESTING agent's own queue
carrying both ids as PAYLOAD, so no thread writes another thread's rows and WebGPU still needs no
atomics;
**Rewire Bond** [`rewireBond`] — atomically MOVE one of
this agent's bonds from one partner to another, the verb graph-rewriting automata name; the
intermediate half-rewired state never exists, and nothing at all is applied if the move cannot
complete. It arrived with the per-agent structural-request QUEUE, which also lets Form / Break
Bond issue SEVERAL ops in one step (the earlier one-request-per-step limit made every
degree-preserving rewrite inexpressible); **Get / Set Bond Attribute** [`getBondAttribute` /
`setBondAttribute`] are the newest — per-EDGE user state read/written by PARTNER ID; a bond is
one object stored at both endpoints, so Set Bond Attribute writes BOTH slots [invariant I2] and
a directed quantity is expressed by STORING an owner/direction value, never by writing the two
sides differently. Form Bond seeds a new bond's values through one input port per bond
attribute. ALL THREE agent targets (JS↔WASM bit-parity; on WebGPU the bond store's slot
stride widens by one word per attribute and is promoted to read_write — with the one caveat that
BOTH endpoints writing the same bond in ONE step is order-undefined there);
**Neighbour Census** [`neighbourCensus`] — the
...
appear in the editor only when their respective model feature is enabled (the 51 agent
...
*Divide Agent* → **Divide Self** — the five field-bridge nodes gain an explicit
...
- **`vector` is ALSO a stored attribute + Local-Variable type** (a per-cell / per-agent
  ...
  **Get Agent Attribute** (by id), and **Transfer Cell Attributes to Neighbor**
...
- **Multi-attribute slots** — the five accessor nodes (**Get/Set (Self) Attribute**,
  **Get Model Attribute**, the by-id **Get Agent Attribute**) take a dynamic number of
...
| 6b | `assertActiveViewer` | Assert Active Output Mapping | Run a branch ONLY while the chosen Attribute→Color mapping is the viewer the simulator is currently showing — so visualization-only work costs nothing when its viewer is off screen. | `I: DO` (flow) / `O: DONE` `O: IF ACTIVE` (flow); config `mappingId` (A→C mappings only) | **Conditional minus the ELSE**, deliberately: `IF ACTIVE` is a real BRANCH port, so sink analysis SINKS the values only it consumes INSIDE the guard (a transparent `next`-style guard would leave the work above it and save nothing). `DONE` keeps its universal meaning and runs unconditionally. The test is always "is this mapping the simulator's ACTIVE viewer", on every entry point. Emits the SAME comparison Set Cell Looks uses — JS `_isV_<safeId>`, WASM's per-step `viewerLocals` i32, WGSL `control.activeViewer` — so the two can never disagree. **Lattice-only** (`LATTICE_ONLY_TYPES`): the WebGPU AGENT shader carries no activeViewer, so shipping it on the Agents graph would be a partial target set. An unset / C→A / no-branch config DROPS the branch on every target; an unknown id is a named error from the shared dangling-reference gate |
...
| 12c | `getGeneration` | Get Generation | Outputs the current generation number, **0-based**: the first step after a Reset is 0. **UNIVERSAL** — cells AND agents read ONE shared counter, so both layers see the same number within a generation. Compose it for cadence (`Get Generation → Math (%) → Compare (==) → If/Then`), for time-varying colour, or for anything that should change with the clock. | `O: Generation` (int) | Pinned semantics: an **Init event reads 0** (Reset zeroes the counter before running it); a **Division Event reads the generation the division happened in**; an **Output Mapping reads the generation about to be computed** (it runs after the increment). Pure + input-free ⇒ loop-invariant and CSE-eligible. Threading costs nothing on a model that omits it — the param / struct field / WASM load simply are not emitted |
...
| 27 | `getRandom` | Get Random | Random binary / integer / decimal (uniform, normal or exponential) / orientation, one option picked uniformly from a wired Options array, a random **vector** of a given length and angular spread, or a random **color**. | `I: P` (float, Binary), `I: Min` `I: Max` (float, inline, Integer + uniform Decimal), `I: Mean` `I: Std Dev` (float, inline, normal/exponential Decimal), `I: Options` (any, isArray) `I: Fallback` (any, inline) (Options), `I: Norm` `I: Angle°` `I: Dir X` `I: Dir Y` `I: Span°` (float, inline, Vector) / `O: Value`; Vector mode instead exposes `O: X` `O: Y` (float) + `O: Vector` (vector); Color mode instead exposes `O: R` `O: G` `O: B` (int) + `O: Color` (color) | **Every parameter is a PORT**, so Min/Max/Mean/σ/Norm/Angle/Span can be driven by a wire. Draw counts (identical on every target): normal = **2** stream advances (Box-Muller, fixed count — never a rejection loop), **color = 3** (one uniform draw per channel, in the order R, G, B), everything else = **1**. Vector: the direction is uniform within ±Span°/2 of a reference — a compass **Angle°** (0° = north = −y, 90° = east = +x, matching the sprite heading convention) or a wired **Dir X/Y**; a zero wired direction falls back to north. Planar (XY) in 3D models too. Color: each channel is `floor(u * 256) & 255`, a uniform integer in 0..255; **no alpha is drawn** (a random opacity is almost never wanted), so the composite `Color` port carries a literal 255. Both composite outputs (`Vector`, `Color`) lower (via `expandComposites`) to this node's OWN component ports, so wiring one costs no extra draw however many consumers it has. Legacy `config.min`/`config.max` are migrated onto the ports on load |
| 28 | `getVariable` | Get Variable | Read a Local Variable's current value (scalar) or its underlying typed array (array variables — consumers iterate like any array source). | `O: Value` (any / **vector**) | Requires `variableId`; output shape (scalar vs array) derived from the variable's `kind`. A **vector**-typed scalar variable retypes `Value` to a composite `vector` port (`vectorPortDims`) and lowers to a Make Vector over its components. Per-cell scratch, reset to `initialValue` each cell |
...
| # | Type | Label | Description | Ports | Notes |
|---|---|---|---|---|---|
| 36 | `arithmeticOperator` | Math | `+ − × ÷ % sqrt pow abs negate floor ceil round max min mean exp log sin cos tan tanh` (`log` = natural log; `round` = `floor(x + 0.5)` on every target — never banker's rounding; `negate` = unary minus, an IEEE sign flip that is bit-identical across targets incl. `-0`/NaN and needs no host import). | `I: X` `I: Y` (num, `Y` hidden on unary ops) / `O: Result` | Unary ops (`sqrt abs negate floor ceil round exp log sin cos tan tanh`) read only `X` |
| 37 | `expression` | Math Expression | Type a math formula instead of wiring many Math nodes — collapses an equation-heavy chain into one node. Operators `+ − × ÷ % ^`, functions `sqrt abs floor ceil round min max pow mod exp log sin cos tan tanh` (`log` = natural log), constants `pi` `e`. Variables come from the input ports. | dynamic `I: a…h` (1–8 ports, **2 by default**, each renamable — the name editors + the port-count stepper sit behind the collapsed-by-default `▸ Inputs` row) / `O: Result` | Parses to a shared AST; JS / WASM / WebGPU lockstep. The node body renders that same AST as typeset maths (stacked fractions, superscripts, radicals, `\|x\|`, upright `mod`) with parentheses re-derived from precedence. Scalar-only — no array/neighbour reductions. Transcendentals use `Math.*` (JS) / native intrinsics (WGSL) / imported host funcs (WASM, alongside `pow`) |
...
| 125 | `logicalExpression` | Logical Expression | Type a boolean formula instead of wiring a chain of Compare + Logic nodes — what Math Expression is to Math. Operators `NOT AND XOR OR` (or `! && ^ \|\|`), the comparisons `< <= > >= == !=` (`=` is `==`), parentheses, numbers, literals `true`/`false` (or `1`/`0`). Precedence `CMP > NOT > AND > XOR > OR`, so `NOT n > 2` is `NOT (n > 2)`; boolean binary operators are left-associative, comparisons do NOT chain (`a < b < c` is an error naming the `AND` rewrite); operator words and literals are case-insensitive. A variable is dual-role — bare it is truthy-tested, in a comparison it is its raw number. No arithmetic (not even negation): compute in a Math Expression node and wire it in. | dynamic `I: a…h` (**`any`** — the same operand typing Compare uses, so numeric sources are offered; 1–8 ports, **2 by default**, each renamable — same collapsed-by-default `▸ Inputs` row as Math Expression; the inline widget stays bool) / `O: Result` (bool) | Parses to its OWN small AST (the boundary with Math Expression's grammar is ARITHMETIC and only arithmetic — this one has no arithmetic, that one has no boolean operators); JS / WASM / WebGPU lockstep on both the cell and agent loops, plus the Overseer driver. The node body renders that AST with `¬ ∧ ⊕ ∨` + `< ≤ > ≥ = ≠` and parentheses re-derived from precedence. Boolean reads are truthy-tested (`≠ 0`) and the result is the project's numeric bool 1/0 — identical to `logicOperator`; comparisons reproduce `statement` (Compare) exactly, f64 on the CPU targets and f32 on the GPU ones, with exact `==` / `!=` |
| 42 | `valueSwitch` | Value Switch | `condition ? ifValue : elseValue`. Pure value, no flow port. **Dual-mode:** scalar selector, array relay, OR composite relay. | `I: Condition` (any) `I: If` (any) `I: Else` (any) / `O: Result` (any) | All inputs optional (inline defaults: condition=false, if=1, else=0). Both branches always evaluate — for short-circuit use flow `conditional` instead. **Array relay:** when BOTH `If` and `Else` are wired to array producers (e.g. two `filterNeighbors`), `Result` is the selected array (feed it to `pickRandomNeighbor` / `arrayElement` / `aggregate` / …). All three targets — JS relays the reference, WASM selects the scratch offset/len (zero-copy), WebGPU copies the chosen branch. **Composite relay:** when BOTH `If` and `Else` carry the SAME composite (`vector` or `color`), `Result` is that composite — wire it into Break Vector / Vector Op / a `vector` attribute's Set Attribute / Apply Force. Lowered by `expandComposites` into one SCALAR Value Switch PER COMPONENT (sharing the one condition), so every target emits it through the verified scalar path with no per-target composite code. Both branches must agree: one composite + one number has no per-component meaning, so the editor refuses to create it and the compiler reports it by name. |
...
| 109 | `breakVector` | Break Vector | Split a `vector` back into its X / Y / Z scalar components (Unreal Break Vector / Blender Separate XYZ). | `I: Vector` (vector) / `O: X` `O: Y` `O: Z` (float) | Multi-output; `Z` hidden in 2D. Lowered to scalars (all targets) |
| 110 | `vectorOp` | Vector Op | Vectorial math on `vector` values: Add / Subtract / Scale (×scalar) / Dot / Cross / Length / Normalize / Distance / Negate / Lerp / **Rotate (2D)** / **Rotate Around Axis (3D)**. Operates on the bundled [x, y, z] so you never touch the components. | `I: A` `I: B` `I: Axis` (vector) `I: Scalar` `I: T` `I: Angle°` (float, inline) / `O: Vector` (vector) `O: Value` (float) | Multi-output; ports shown per op (`hiddenPorts`). Vector-returning ops → `Vector`; Dot / Length / Distance → `Value`. **Angles are in DEGREES**; + rotates from +X toward +Y = clockwise on screen (rows grow downward). **Rotate** spins the XY plane about Z and passes Z through (so it works in 2D and 3D); **Rotate Around Axis** is Rodrigues about an arbitrary axis, offered only in a 3D model (a zero axis normalises to 0 via the ÷0→0 guard, degenerating to `v·cosθ`). Lowered to `arithmeticOperator` trees (deg→rad × then sin/cos) → all 3 targets |
...
| # | Type | Label | Description | Ports | Notes |
|---|---|---|---|---|---|
| 52 | `setVariable` | Set Variable | Assign a value to a scalar Local Variable. | `I: DO` (flow) `I: Value` (any / **vector**) / — | Requires `variableId`; rejects array variables (use Set Array Element). A **vector**-typed scalar variable retypes `Value` to a composite `vector` port (the inline number widget drops) and lowers to a Break Vector + one component write per axis |
...
| 54 | `setAttribute` | Set Attribute | Write a value to an attribute of the current CELL — or, on the AGENTS graph, of an agent. The optional `Agent` input is the standard targeting convention, widened to SCALAR-OR-ARRAY: **empty = the current agent**, a scalar id = that agent (signalling a neighbour, or configuring a Create Agent handle), an id **ARRAY** (Get Nearby / Bonded / Filter Agents…) = EVERY agent in it. Extra attribute slots write several attributes in slot order, each with its own `value_N` input + type-adaptive inline widget; a wired `Agent` FANS OUT to every slot, so one node can set N attributes on a whole group. | `I: DO` (flow) `I: Agent` (int, **arrayCapable**, Agents graph only) `I: Value` + `I: value_N` per extra slot / `O: NEXT` (flow) | Multi-slot expands to a linear single-set flow splice pre-compile. The `Agent` port is hidden on the Cells graph (`hiddenPorts`, graph-aware); its scalar-vs-array shape is a COMPILE-time decision (the shared `isAgentIdArraySource`), never a runtime test. **A WIRED id is a cross-agent OVERWRITE**: rejected in SYNCHRONOUS agent mode and by the WebGPU agent gate unless it comes from a Create Agent handle — such a model runs on JS/WASM. A scalar id takes the relaxed range-only guard (it may be a staged spawn handle); each id of an ARRAY keeps the strict live-agent guard. Absorbed the retired `setAgentAttribute` ("Set Attribute (by ID)") and `setAgentsAttribute` ("Set Agents Attribute"); legacy files are migrated on load |
...
| 111 | `makeColor` | Make Color | Bundle R / G / B / A channels into a single `color` value (the Unreal Make Color / Blender Combine Color). | `I: R` `I: G` `I: B` `I: A` (float, inline; A defaults 255) / `O: Color` (color) | Lowered to scalars by `expandComposites` → runs on all 3 targets |
| 112 | `breakColor` | Break Color | Split a `color` back into its R / G / B / A channels (Unreal Break Color / Blender Separate Color). | `I: Color` (color) / `O: R` `O: G` `O: B` `O: A` (float) | Multi-output. Lowered to scalars (all targets) |
...
|---|---|---|---|---|---|---|
| 69 | `behaviourStep` | Behaviour Step | `event` | The per-agent update entry point — the agent analogue of `step`. The compiler loops `idx < highWater` over the agent SoA (skipping dead slots) and runs the DO flow once per live agent each generation. | `O: DO` (flow), `O: X` `O: Y` (float), `O: Radius` `O: Area` (float), `O: Bond Degree` `O: Age` (int); 3D exposes `O: Z` and `O: Volume` (float) | Singleton (one per Agents graph). Value-outs resolve via the `_v<id>_<port>` convention. The `Type` output was removed — agents have no built-in type. `Z` + `Volume` hidden in 2D. **`Area` is the agent's extent in the model's own dimension** — the disc area πr² in 2D, the sphere's SURFACE area 4πr² in 3D; `Volume` is the enclosed (4/3)πr³. The 2D expression is unchanged, so every 2D model's emit is byte-identical; the JS and WASM emitters mirror each other's operand ORDER (f64 multiplication is not associative). `Volume` is the one value-out that is USAGE-GATED on JS (emitted only when wired), so an unwired port cannot move a byte of any existing model's emitted code; WASM/WGSL are per-port emitters and gated by construction |
| 69b | `periodicStep` | Agent Periodic Step | `event` | A **PER-AGENT** entry point — its DO chain runs FOR EVERY LIVE AGENT, but only on generations where `generation % Period === Phase`. (The GLOBAL once-per-firing counterpart is **Population Periodic Event**; the type id is unchanged, only the label — the old unqualified "Periodic Step" read as global.) **Several are allowed per graph** — that is the point: two at Period 2, phases 0 and 1, alternate state updates and rewrites (gating the state update and the rewrite on the SAME tick is what makes a periodic automaton faithful); three at 1 / 5 / 50 give a fast, a medium and a slow clock in one model. Coexists with a plain Behaviour Step, whose chain keeps running every generation. | `O: DO` (flow), `O: Step Index` (int = ⌊generation / Period⌋) | **NOT a singleton.** Lowered before compile (`periodicExpand.ts`) into `Get Generation → Math(%) → Compare(==) → If/Then` hung off ONE synthesized-or-reused `behaviourStep`, sequenced so the unconditional chain runs first — so all three targets work by construction with zero per-target emit. `Period` clamps to ≥ 1, `Phase` folds into [0, Period). `Step Index` is synthesized only when consumed |
| 70 | `divisionEvent` | Division Event | `event` | Runs once per daughter right after a division, so daughters can be given different attribute values (asymmetric inheritance). Both daughters start with the mother's attributes; `Set Attribute` here overwrites them. **It runs AFTER the split has committed** — both daughters exist, both are alive, both ids are known — which is why it is also where a daughter's BONDS are authored (D4). | `O: DO` (flow), `O: Daughter #` (int), `O: Sibling` (int), `O: Axis X` `O: Axis Y` (float), `O: Area` (float); 3D exposes `O: Axis Z` and `O: Volume` (float) | Singleton. `Daughter #` 0 = reused mother slot, 1 = new slot; `Axis X/Y` = the engine's chosen division axis. **`Sibling` (D3)** = the OTHER daughter's agent id, so ONE invocation can configure BOTH daughters by id (their strict alive guard is satisfied here) or bond them together; **usage-gated** — the trailing ABI scalar and its alias are emitted only when the port is wired, so a model that ignores it is byte-identical. `Area` is the dimension's own extent — πr² disc in 2D, 4πr² sphere surface in 3D (see Behaviour Step); `Volume` = (4/3)πr³, 3D-only + usage-gated. The event CANNOT see the Behaviour Step's Local Variables (per-step scratch, gone by the structural phase) — carry context in an AGENT ATTRIBUTE written just before Divide Agent; both daughters inherit it verbatim |
...
| 74 | `neighbourDensity` | Neighbour Density | `data` | How many **other** agents are nearby (local crowding). First-class — distinct from bond degree (a free agent has density but no bonds). **Radius unwired / 0** = the engine's interaction cutoff `interactionRange × (r_i + r_j)` (relative, free, one generation stale); **Radius wired or > 0** = a fresh count at that absolute distance. | `I: Radius` (float, inline, 0 = engine range) · `O: Density` (int) | Unwired: reads `_agentDensity[idx]`. Active Radius: LOWERED before compile to `Get Nearby Agents(radius) → Array Length` (`densityExpand.ts`) — zero per-target emit; costs one agent-array-producer slot and must stay ≤ Neighbour Query Radius |
...
| 78 | `setTargetRadius` | Set Target Radius | `output` | Set the radius an agent grows toward; the engine ramps the actual radius each step (at the model's growth rate). A grown agent is what divides. **Agent empty = self, else that agent by id** — Set Agent Radius sets the CURRENT radius, this sets the GROWTH TARGET. | `I: DO` `I: Agent` (integer, optional) `I: Target` (float, inline) / `O: NEXT` (flow) | Writes `_agentTargetRadius[id]`. A WIRED id is a cross-agent OVERWRITE ⇒ rejected in Synchronous agent mode + by the WebGPU agent gate (non-spawn) |
| 79 | `formBond` | Form Bond | `output` | Request a (symmetric) bond between **two agents** — applied in the post-step structural phase. No-op if already bonded, an endpoint is dead, or either list is full. | `I: DO` `I: Agent A (self)` (int, **optional** — unwired ⇒ THIS agent) `I: Target` (int) `I: Rest Length` `I: Stiffness` (float, inline) + **one input per BOND ATTRIBUTE** (labelled with its name, type-adaptive inline widget) / `O: NEXT` (flow) | NOT async-only. Rest Length 0 = contact distance; Stiffness 0 = the model's λ. Auto-bond forms bonds by distance with no node. The bond-attribute inputs seed the new bond's INITIAL values (both slots — I2); absent ⇒ each attribute's default, and a model with no bond attributes emits byte-identical code. **P4: requests are QUEUED** — an agent may form SEVERAL bonds in one step (depth in Properties → Bond-Graph Agents; ops past it are rejected whole with a notice). **Agent A defaults to self**: wiring it LOWERS the op to the *between* encoding (the negated break lane), so one node covers both "bond me to X" and "bond X to Y"; wiring Get Self Handle is indistinguishable from leaving it unwired, and an UNWIRED node emits the historical code byte-for-byte on all three agent targets. **ALL-OR-NOTHING** in the paired form (invariant I5): out of range / dead / `A === Target` / already bonded / **either** list full ⇒ nothing at all; rest length 0 ⇒ the NAMED pair's contact distance. A retired `formBondBetween` node migrates onto this one (`agentB` → `Target`) **D4 — valid in the DIVISION EVENT too**: a request queued there is applied by a SECOND `drainAgentBondRequests` pass run right after every division event, so a daughter can be bonded (or unbonded / rewired) IN THE GENERATION IT IS BORN. Safe because the drain zeroes each entry before decoding it, so the second pass re-applies nothing, and both daughters' queues are provably empty when their event runs. The `division` ABI's queue block is usage-gated on that subtree, so a model that does not use one is byte-identical. |
| 80 | `breakBond` | Break Bond | `output` | Request that the bond between two agents be removed (symmetric) — applied post-step. | `I: DO` `I: Agent A (self)` (int, OPTIONAL) `I: Target` (int) / `O: NEXT` (flow) | Typically fed by `forEachBond`'s `Partner` (e.g. break over-strained bonds). **`Agent A` unwired = THIS agent**; wired it cuts the bond between two OTHER agents (the third-party cut — lowers to the both-lanes-negative queue encoding, all 3 agent targets). A cut that would break nothing touches nothing (I5). **P4: requests are QUEUED** — an agent may break several bonds in one step **D4 — valid in the DIVISION EVENT too**: a request queued there is applied by a SECOND `drainAgentBondRequests` pass run right after every division event, so a daughter can be bonded (or unbonded / rewired) IN THE GENERATION IT IS BORN. Safe because the drain zeroes each entry before decoding it, so the second pass re-applies nothing, and both daughters' queues are provably empty when their event runs. The `division` ABI's queue block is usage-gated on that subtree, so a model that does not use one is byte-identical. |
...
| 82 | `divideAgent` | Divide Agent | `output` | Request the agent divide into two daughters along its **tension axis** (the net-stretch direction of its bonds — a closed-form 2×2 eigensolve). A **`partition`** config says which bonds each daughter inherits: `tension` (geometric, the DEFAULT and byte-identical to pre-P5), `alternate` (A, B, A, B… in slot order), or `byBondAttribute` (a named bond attribute picks the daughter — bool false→A/true→B, tag = a per-option A/B table, integer/decimal = a threshold). `daughterBond` (decision D4) is `auto` (only when the mother was bonded — the pre-P5 rule), `always` or `never`. A **`conserve`** config (3D only, the row is hidden in 2D) says what the daughter RADII preserve: `area` (the DEFAULT — `rA² + rB² = r²`, i.e. `r·√f`, the historical split in BOTH dimensions) or `volume` (`rA³ + rB³ = r³`, i.e. `r·∛f`). The area split is not volume-conserving in 3D — at a symmetric split ~29 % of the volume disappears. | `I: DO` `I: Axis X` `I: Axis Y` (float) `I: Asymmetry` (float, inline) / `O: NEXT` (flow) | Applied post-step. Overflow (maxAgents / maxBonds) rejects the **whole** division (never half-divided) — in EVERY partition mode. Wire Axis X/Y to override (e.g. up a gradient); `axisSource` is "tension" (a sphere has no shape long-axis). Bond attributes travel WITH their bond through the partition (never re-initialised). An unresolvable partition attribute falls back to `tension` **and** raises a validation badge. **ALL THREE agent targets**: the partition is per-NODE but applied by the ENGINE, so the compiler collects one table entry per distinct spec and every target writes its 1-based code into the EXISTING `divideRequest` cell — no new store lane, no layout change ([dividePartition.ts](../src/modeler/vpl/compiler/dividePartition.ts)). `conserve` rides the SAME table (its key gains a `|volume` suffix ONLY for the non-default value, so every pre-existing key — and therefore every assigned code — is untouched), and both the spec resolver and the engine coerce `volume` → `area` in a 2D world |
| 83 | `killAgent` | Kill Agent | `output` | Request that an agent die — the slot is recycled, ALL its bonds (both directions) are broken, and its slot epoch is bumped (the dangling-bond ABI). **Agent empty = self, else that agent by id** (PREDATION — "consume" a neighbour). | `I: DO` `I: Agent` (integer, optional) / `O: NEXT` (flow) | Applied post-step. A wired kill is a flag set to the CONSTANT 1 — idempotent + order-independent, so it is NOT gated in Synchronous mode and NOT rejected by the WebGPU agent gate (unlike the by-id OVERWRITE setters) |
...
| 86a | `getAgentsInView` | Get Agents In View | `data` | The DIRECTIONAL Get Nearby Agents — only the nearby agents inside a heading-relative vision CONE. Sensing capability. The cone test rides the same hash gather (zero new SoA/ABI); `cosHalf` is a compile-time literal (JS↔WASM bit-parity); zero heading ⇒ omnidirectional. | `I: Radius` (float, inline), `Heading X/Y/Z` (float, inline — used by the **Wired** source AND the lowered target of the **Facing** source, Z in 3D) / `O: Agents` (int **array**) · config `Half-angle°` + `Heading` (**Velocity / Wired / Facing**; Facing adds a vector-agent-attribute picker + needs the **Orientation** capability — lowered via `lowerFacingSource`: Get Cell Attribute[vector facing attr] → Break Vector → Heading inputs) + `Cone color` (optional `visionColor` `#rrggbb` — DISPLAY-ONLY: tints this node's wedges in the simulator's Show-vision overlay; no compiler on any target reads it) | Per-agent (never hoisted). Behaviour-graph only (needs the spatial hash). 2D + 3D |
| 86b | `senseHemifield` | Sense Hemifield | `data` | The Braitenberg LEFT/RIGHT sensor — the SAME FOV-cone gather as Get Agents In View, but each in-view neighbour is SPLIT by the sign of the heading-relative cross product (2D `hx·dy−hy·dx`; 3D triple product with a +Z up-ref, swapped to +Y near-vertical). Steer by `Left − Right`, or ACT on a side through its two id ARRAYS. Multi-output value node + a CONDITIONAL array producer (an unwired array port emits nothing at all, on every target). | `I: Radius` (float, inline), `Heading X/Y/Z` (float, inline — used by **Wired** AND the lowered **Facing** source, Z in 3D) / `O: Left Count`, `Right Count` (int), `Left Agents`, `Right Agents` (int **array** — the same in-view set the counts count, partitioned, so `length === count`; feed For Each In Array / Get Agents Attribute / Filter Agents / Aggregate) · config `Half-angle°` (default 90 = a 180° front hemisphere) + `Heading` (**Velocity / Wired / Facing**; Facing = a stored vector agent-attribute, needs the **Orientation** capability) + `Cone color` (optional `visionColor` `#rrggbb` — DISPLAY-ONLY: tints this node's wedges in the simulator's Show-vision overlay; no compiler on any target reads it) | Per-agent (never hoisted). Behaviour-graph only. 2D + 3D |
| 87 | `getAgentPosition` | Get Agent Position | `data` | An agent's `(X, Y[, Z])`, with an **Absolute / Relative** `mode`. Absolute = the raw position — **Agent empty = self**, else that agent by id (field seeding). Relative = the **torus-shortest** vector `target − reference` (folded like the engine); the `Reference` input likewise defaults to **self** when unwired, or any agent when wired (offset between two agents). Wrap-correct alternative to Get Agent Offset (which keeps a Distance output). | `I: Agent` (int, optional); relative adds `I: Reference` (int) / `O: X` `O: Y` (float); 3D adds `O: Z` | Multi-output. `Reference` hidden in absolute mode, `Z` in 2D. All three targets (JS/WASM/WebGPU). Unwired ⇒ an unguarded `_agentX[idx]` self read; a WIRED id keeps the range guard byte-for-byte. In relative mode the sentinel is kept: with `Reference` already self, an unwired `Agent` is self−self = the same zero vector the failed guard gives |
| 94 | `getAgentOffset` | Get Agent Offset | `data` | The **torus-shortest** displacement `(dX, dY)` + `Distance` from THIS agent to a target by id. Use this (or Get Agent Position's relative mode) — NOT hand-subtracting two raw positions — for cohesion / separation / steer-toward-neighbour so it stays correct across a torus seam. `dX = target − self` (TOWARD target, matching the engine's `+k·dx` attractive sign). | `I: Agent` (int, **REQUIRED**) / `O: dX` `O: dY` `O: Distance` (float) | Multi-output; folds the delta to torus-shortest reading `_fieldW`/`_fieldH`/`_fieldBoundaryTorus`. CSE-eligible (positions read-only within a step). All three targets (JS/WASM/WebGPU). The `Agent` id is the one by-id READER still badged when unwired — an offset to yourself is identically zero, so there is no self meaning to default to |
| 88 | `getAgentAttribute` | Get Agent Attribute | `data` | Read an agent's attribute — **Agent empty = self**, else that agent by id (the agent analogue of Get Neighbor Attribute By Index) — differential adhesion, contact inhibition, signalling. Extra attribute slots read several of that agent's attributes through the ONE shared Agent input (fanned out at expansion). | `I: Agent` (int, optional) / `O: Value` (any) + `O: value_N` per extra slot | Requires `attributeId` (+ each slot's `attr_N`); the `Agent` id is OPTIONAL and NOT badged. Unwired ⇒ an unguarded `r_<attr>[idx]` self read (exactly what Get Self Attribute emits); a WIRED id keeps the `-1`-sentinel range guard byte-for-byte. **Get Self Attribute stays the preferred self reader** — it is CSE-eligible and loop-invariant-hoistable, while this node is impure (a neighbour write can mutate it) whichever way its id is wired |
| 89 | `getAgentRadius` | Get Agent Radius | `data` | An agent's radius — **Agent empty = self**, else that agent by id — for size-aware neighbour interactions. | `I: Agent` (int, optional) / `O: Radius` (float) | Unwired ⇒ an unguarded `_agentRadius[idx]` self read; a WIRED id keeps the range guard byte-for-byte. `Get Radius` remains the dedicated self reader |
...
| 119 | `neighbourCensus` | Neighbour Census | `aggregation` | The **multiset of neighbour states** — one labelled INTEGER output per state value of a tag/bool AGENT attribute, plus `Total`. The only legal input to a homogeneous GRAPH rule (a node cannot name its neighbours, so it may read only an order-independent, degree-tolerant aggregate). Replaces `Get Bonded Agents → Get Agents Attribute → Count Matching` wired once per state value. | `I: Radius` (float, inline — `nearby` source only) / `O: <option name>` (int) **per state value**, `O: Total` (int) | **Pure editor sugar** — a shared pre-compile pass (`expandNeighbourCensus`) lowers it to the gather + one Count Matching per **consumed** state port (+ Array Length for `Total`), so it reuses those emitters on all three targets (no new per-target code) and needs no supported-types entry. `source` = `bonded` (the 1-ring, default) or `nearby` (a radius). Requires a tag/bool agent attribute; the count ports are DYNAMIC (rebuilt from its options). Capability: Bonds **or** Sensing |
| 120 | `getBondAttribute` | Get Bond Attribute | `data` | Read a BOND's user attribute by PARTNER ID — per-EDGE state (the value belongs to the edge, not to either endpoint). Feed `Partner` from For Each Bond / Get Bonded Agents. Bonds are symmetric, so either endpoint reads the same value. | `I: Partner` (int) / `O: Value` (any) | Requires `attributeId`. No bond with that partner ⇒ the attribute's **default** (never NaN). Per-agent (never hoisted) + impure (a bond attribute is mutable mid-step, and compaction moves slots). Capability: Bonds. **ALL THREE agent targets** (JS + WASM bit-parity; WebGPU reads its slot word out of the stride-widened bond store) |
| 121 | `setBondAttribute` | Set Bond Attribute | `output` | Write a BOND's user attribute by partner id. **Writes BOTH endpoints' slots** — a bond is one object stored twice, and invariant I2 requires them to agree, so there is no "my side". A directed quantity is expressed by STORING an owner/direction value (e.g. an `ownerId` bond attribute compared against Get Self Handle), never by writing the two sides differently. | `I: DO` `I: Partner` (int) `I: Value` (float, inline) / `O: NEXT` (flow) | Requires `attributeId`. No such bond ⇒ no-op; the partner side is range + alive guarded. A bond formed THIS step doesn't exist yet (Form Bond applies in the structural phase) — seed a new bond's values on Form Bond itself. Capability: Bonds. **ALL THREE agent targets**; on WebGPU, when BOTH endpoints write the same bond in ONE step the winner is **order-undefined** (write from one side, or keep the rule symmetric — measured exact for both of those, P3) |
| 122 | `rewireBond` | Rewire Bond | `output` | **Atomically MOVE one of this agent's bonds**: break the bond to `From` and form one to `To` as ONE operation. The verb graph-rewriting automata actually name — the intermediate half-rewired state never exists, so a degree-preserving rule (triangle split, edge swap, pair annihilation) holds its invariant at EVERY generation, not merely between them. | `I: DO` `I: From` (int) `I: To` (int) `I: Rest Length` (float, inline) `I: Stiffness` (float, inline) + one initial-value port per bond attribute / `O: NEXT` (flow) | Applied in the post-step structural phase, via the per-agent request QUEUE (so an agent can issue several ops per step — depth in Properties → Bond-Graph Agents). **ALL-OR-NOTHING** (invariant I5): if there is no bond to `From`, or `To` is dead / out of range / this agent, or `To`'s list is full, **nothing** is applied — never a bare break. `To === From` re-forms the same edge with the new parameters. Capability: Bonds. **ALL THREE agent targets** (JS↔WASM bit-parity; on WebGPU each thread appends only to its OWN queue rows, so no atomics are involved) **D4 — valid in the DIVISION EVENT too**: a request queued there is applied by a SECOND `drainAgentBondRequests` pass run right after every division event, so a daughter can be bonded (or unbonded / rewired) IN THE GENERATION IT IS BORN. Safe because the drain zeroes each entry before decoding it, so the second pass re-applies nothing, and both daughters' queues are provably empty when their event runs. The `division` ABI's queue block is usage-gated on that subtree, so a model that does not use one is byte-identical. |
| 124 | `transferBond` | Transfer Bond | `output` | **Hand this agent's bond with `Partner` over to `New Partner`, rewriting Partner's slot IN PLACE.** The order-preserving sibling of Rewire Bond, which is break+form at the REQUESTER and therefore compacts (scrambles) the third party's list. Slot order decides which neighbour each daughter of a split inherits, so it propagates into the structure — and the embedding — forever. The bond KEEPS its rest length, stiffness and every bond attribute: it is the SAME edge re-pointed, not a fresh one, which is also what makes I2 hold by construction (the mirror slot appended at the new partner is stamped with those same values). | `I: DO` `I: Partner` (int) `I: New Partner` (int) / `O: NEXT` (flow) | Applied in the post-step structural phase via the REQUESTING agent's own request QUEUE (the two ids are PAYLOAD, not addresses — **WebGPU still needs no atomics**). **ALL-OR-NOTHING** (invariant I5): no-op unless this agent is actually bonded to Partner, New Partner is live / in range / distinct from both, Partner is NOT already bonded to New Partner (that would be a double edge), and New Partner has a free slot. Degrees after: Partner unchanged, this agent −1, New Partner +1. Capability: Bonds. **ALL THREE agent targets** (JS↔WASM bit-parity) **D4 — valid in the DIVISION EVENT too**: a request queued there is applied by a SECOND `drainAgentBondRequests` pass run right after every division event, so a daughter can be bonded (or unbonded / rewired) IN THE GENERATION IT IS BORN. Safe because the drain zeroes each entry before decoding it, so the second pass re-applies nothing, and both daughters' queues are provably empty when their event runs. The `division` ABI's queue block is usage-gated on that subtree, so a model that does not use one is byte-identical. |
...
| 101 | `setVelocity` | Set Velocity | `output` | Set an agent's velocity directly — the momentum companion to Apply Force (seeds the integration velocity rather than accumulating a force). **Agent empty = self, else that agent by id** (mirrors Get Velocity) — e.g. a knock-back. | `I: DO` `I: Agent` (integer, optional) `I: Vx` `I: Vy` (float, inline); 3D adds `I: Vz` / `O: NEXT` (flow) | Only meaningful when the model's Momentum > 0 (overdamped mode recomputes velocity from the force each step — use Apply Force there). Writes `_agentVX/VY[id]`. A WIRED id is a cross-agent OVERWRITE ⇒ rejected in Synchronous agent mode + by the WebGPU agent gate (non-spawn); use Apply Force To Agent for an order-independent alternative. `Vz` hidden in 2D (and dropped in the Init Event — no `_agentVZ` param there) |
| 102 | `agentInit` | Agent Init Event | `event` | A once-per-Reset setup root for the Agents graph (the agent analogue of the cell Init Event). Wire a Loop inside DO and spawn the initial population with Create Agent → set-by-handle → Add Agent To World. | `O: DO` (flow), `O: World Width` `O: World Height` (float); 3D adds `O: World Depth`; `O: Seed Index Base` (int) | Singleton. Runs exactly once (NOT per-agent, no `idx`); composes additively with the config `seedCount`. Multi-output. `World Depth` hidden in 2D |
| 102b | `agentPeriodic` | Population Periodic Event | `event` | Runs **ONCE GLOBALLY** (not per agent, **no `self`**) on generations where `generation % Period === Phase` — the periodic counterpart to the Agent Init Event, and the GLOBAL sibling of the per-agent **Agent Periodic Step**. Spawn a wave with **Create Agent → set-by-handle → Add Agent To World**, retire agents by id, sweep an attribute, or read an indicator and fire a **Stop Event**. Fires at the TOP of the generation, so a newborn is inside the agent loop's bound and behaves + integrates THAT generation (like an Agent-Init-Event agent, unlike a Behaviour-Step spawn). | `O: DO` (flow), `O: World Width` `O: World Height` (int), `O: World Depth` (int, 3D only), `O: Step Index` (int), `O: Seed Index Base` (int — `highWater` at the firing) | Compiles to ONE JS function the worker executes on EVERY compile target (JS / WASM / WebGPU) — the Grid Init Event architecture, so the WASM/WebGPU STEP compilers never see this root and there is no per-target emit, no supported-type set to widen and no gate to change. **NOT a singleton** (several cadences per graph). `Period` clamps to ≥ 1 and `Phase` folds into [0, Period) through the SAME shared `periodicParams` the Agent Periodic Step uses. Compiled against the **Agent Init Event's own ABI kind** (`buildAgentInitParams`) and called through the SAME `buildAgentInitArgs` with the SAME grow-only spawn closures, so it adds no ABI kind, no arg builder and no descriptor arm. It **BLOCKS the GPU-resident batch** (a resident batch encodes N generations into one submit with no CPU touch point — exactly what a per-generation event needs): a class-F cost, never an error. Agents-graph only |
| 103 | `createAgent` | Create Agent | `output` | Phase 1 of the two-phase spawn — allocate a STAGED agent (`alive=0`) at a position and return its `Handle` (the new id, or `-1` on overflow) so you can set its attributes before committing. | `I: DO` `I: X` `I: Y` (float, inline) `I: Radius` (float, inline); 3D adds `I: Z` / `O: NEXT` (flow), `O: Handle` (int) | Only meaningful inside the Agent Init Event (v1 is init-only). Multi-output. A handle never Added is swept back to the free-list at the end of init. (The `Type` input was removed — agents have no built-in type; set agent attributes instead.) `Z` hidden in 2D |
| 104 | `addAgentToWorld` | Add Agent To World | `output` | Phase 2 of the two-phase spawn — commit a staged agent (from Create Agent's handle), marking it live (`alive=1`, liveCount++) so the simulation processes it. | `I: DO` `I: Handle` (int) / `O: NEXT` (flow) | Only meaningful inside the Agent Init Event. Calls the `_agentAddToWorld` host closure |
| 105 | `setAgentPosition` | Set Agent Position | `output` | Set an agent's position by id — a spawn helper for a staged Create Agent handle (also works on a live agent). | `I: DO` `I: Agent` (int, **REQUIRED**) `I: X` `I: Y` (float, inline); 3D adds `I: Z` / `O: NEXT` (flow) | In the Init Event the guard is range-only (a staged agent is `alive=0`); elsewhere it requires a live agent. `Z` hidden in 2D. An unwired `Agent` writes NOTHING — badged, and deliberately NOT defaulted to self: the node exists to configure a Create Agent handle, so a self default would silently retarget an existing rule |
| 106 | `setAgentRadius` | Set Agent Radius | `output` | Set an agent's radius (and growth target) by id — a spawn helper for a staged Create Agent handle (also works on a live agent). | `I: DO` `I: Agent` (int, **REQUIRED**) `I: Radius` (float, inline) / `O: NEXT` (flow) | Writes both `_agentRadius` and `_agentTargetRadius` so the growth ramp doesn't drag it away. Unwired ⇒ a silent no-op ⇒ badged (same reasoning as Set Agent Position; use Set Target Radius for this agent) |
| 107 | `getSelfHandle` | Get Self Handle | `data` | The CURRENT agent's own handle (its id = the loop `idx`) — pass it to the by-id nodes (Get Agent Attribute, a wired Set Attribute, Get Agent Position/Offset/Radius, Form/Break Bond) so a neighbour can reference back to me, or to compare a Get Nearby Agents id against self. | `O: Handle` (int) | Emits `idx`. Runs on all three agent targets (JS / WASM / WebGPU) |
| 113 | `agentInputMapping` | Agent Input Mapping (C→A) | `event` | The agent analogue of `inputColor`, over an `isAttributeToColor === false` entry in `model.agentMappings`. Its mapping's **brush KIND** decides the shape: **Editor** (the default) roots a SINGLE-agent graph run once per PAINTED agent — the DO chain writes that agent's state (Set Attribute, Set Agent Radius, Set Velocity, …) and may spawn agents around it or remove it; **Spawner** roots a once-per-BRUSH-APPLICATION graph with NO `self`, which receives the brush geometry and CREATES agents (Create Agent → set-by-handle → Add Agent To World). The brush hands both the mapping's declared PARAMETERS (or, when it declares none, the legacy `R`/`G`/`B` colour). | `O: DO` (flow) + **DYNAMIC** value outputs, one per declared parameter (`color` → three); legacy ⇒ `O: R/G/B` (integer). **Spawner kind PREPENDS** `O: Brush X`, `Brush Y`, `Brush Radius` (+ `Brush Z` in 3D) | Requires a C→A agent `mappingId` (the picker + the badge both direction-filter). NOT a singleton — one per mapping. Compiled to a JS fn run on the CPU on EVERY agent target (event tempo, the agent colour-pass / Division Event posture). A SPAWNER graph has no `self`, so a per-agent reader in its cone is badged (the Agent-Init-Event footgun class) |
...
| 116b | `gridPeriodic` | Grid Periodic Event | `event` | Runs **ONCE GLOBALLY** (not per cell) on generations where `generation % Period === Phase` — the periodic counterpart to the Grid Init Event. Loop here + **Set Cell (at Position)** to add substrate / seed a wave / redraw a region, or read an indicator and fire a **Stop Event**. Fires at the TOP of the generation, BEFORE the agent step and BEFORE the cell step, so what it writes is visible to that same generation's rule. | `O: DO` (flow), `O: Width` `O: Height` (int), `O: Depth` (int, 3D only), `O: Step Index` (int = ⌊generation / Period⌋) | Compiles to ONE JS function the worker executes on EVERY compile target (JS / WASM / WebGPU) — the Grid Init Event architecture, so the WASM/WebGPU STEP compilers never see this root and there is no per-target emit, no supported-type set to widen and no gate to change. **NOT a singleton** (several cadences per graph). `Period` clamps to ≥ 1 and `Phase` folds into [0, Period) through the SAME shared `periodicParams` the Agent Periodic Step uses. On the **WebGPU grid** target the worker reads the GPU attrs back BEFORE a firing and uploads after, so an evolved board is never clobbered by a stale CPU mirror — a cost paid ONLY on firing generations. Under "Skip Isolated Empty Cells" a firing rebuilds the active set (it may write any cell). Cells-graph only |
...
(Loop, If, For Each, Switch, Sequence, Math, Math Expression, Compare, Logic, Logical Expression, Get Random,
