...
**Scope:** 123 registry node types across 7 categories (event, flow, data, logic, aggregation,
output, color) — 3 hidden from the Add Node menu (`macro` / `macroInput` / `macroOutput`),
leaving **120 selectable** (Agent Capability Profiles added **Get Age** [Lifespan] + the two
Sensing nodes **Get Agents In View** [the directional vision cone] + **Sense Hemifield** [the
Braitenberg L/R split of that cone] + STEP 5a's **Spawn Agent** + **Spawn Event** [Population·Birth]).
Indicator
...
> **Bond-Graph Agents note:** these 42 nodes form a second, off-lattice rule world.
...
| 122 | `spawnEvent` | Spawn Event | `event` | STEP 5a Population·Birth — runs once per agent born via Spawn Agent (the birth analogue of Division Event). Set the child's attributes from where it was born / who spawned it. The child already carries the parent's inherited attributes (unless Inherit was off). | `O: DO` (flow), `O: My X` `O: My Y` (float), `O: My Radius` (float), `O: Parent` (int), `O: Parent X` `O: Parent Y` (float); 3D adds `O: My Z` `O: Parent Z` | Singleton. Optional (absent ⇒ the child keeps its inherited/default attrs). CPU-on-JS root (runs the same on every agent target). `My Z`/`Parent Z` hidden in 2D |
...
| 123 | `spawnAgent` | Spawn Agent | `output` | STEP 5a Population·Birth — request a new agent be born at a position this step (eggs / offspring / projectiles). Distinct from the init-time Create Agent + Add To World. Applied in the post-step CPU structural phase (mirrors Divide Agent), so it runs on all 3 agent targets. | `I: DO` `I: X` `I: Y` (float, inline) `I: Radius` (float, inline); 3D adds `I: Z` / `O: NEXT` (flow) | Writes a per-agent spawn request; the structural phase allocs + inherits the parent's attrs (config `inheritAttributes`, default true). One spawn/agent/step (v1). Overflow at `maxAgents` rejects + surfaces a notice. A Spawn Event reassigns the child's attrs. `Z` hidden in 2D. Requires the **Population·Birth** capability |
