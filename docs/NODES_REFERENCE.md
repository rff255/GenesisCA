...
no value ports beyond the two ids. Encoding-wise it is the mirror image of Form Bond Between: the
op kind rides the sign of the FORM lane instead of the break lane, so it costs no new field and
moves no baked offset. The previous pair was the **rule cadence**
...
vocabulary. (**Form Bond** itself now names BOTH ends: its optional `Agent A` port defaults to
THIS agent when unwired — wiring it lowers the op to the Form Between encoding, so one node covers
both "bond me to X" and "bond X to Y". **Form Bond Between** [`formBondBetween`] remains the
explicit two-id verb — bond two OTHER agents by id, the third-party edge a self-to-target Form Bond
cannot make. It exists
...
without it the split has to span two generations and the intermediate state violates the very
invariant the rule preserves. The request rides the REQUESTING agent's own queue carrying both
ids as PAYLOAD, so no thread writes another thread's rows and WebGPU still needs no atomics;
...
| 124 | `formBondBetween` | Form Bond Between | `output` | **Bond two OTHER agents, by id** — the third-party edge a self-to-target Form Bond cannot make. The motivating case is the cubic **triangle split**: its `v₂–v₃` edge joins two agents the rule CREATED THIS GENERATION, so neither is `self` and neither runs its own behaviour until the next step. Without it the split spans two generations and the intermediate state has degree-2 nodes and `E ≠ 3N/2` — violating exactly the invariant the rule exists to preserve. | `I: DO` `I: Agent A` (int) `I: Agent B` (int) `I: Rest Length` (float, inline) `I: Stiffness` (float, inline) + one initial-value port per bond attribute / `O: NEXT` (flow) | Applied in the post-step structural phase via the REQUESTING agent's own request QUEUE (the two ids are PAYLOAD, not addresses — so no thread writes another thread's rows and **WebGPU still needs no atomics**). **ALL-OR-NOTHING** (invariant I5): out of range / dead / `A === B` / already bonded / **either** list full ⇒ nothing at all. Symmetric with identical rest length, stiffness and bond attributes on both sides (I2); rest length 0 ⇒ the NAMED pair's contact distance. Capability: Bonds. **ALL THREE agent targets** (JS↔WASM bit-parity) |
