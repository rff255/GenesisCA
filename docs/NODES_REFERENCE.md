...
| 87 | `getAgentPosition` | Get Agent Position | `data` | A specific agent's **raw** `(X, Y)` by id (an absolute position — for field seeding). For relative neighbour vectors use Get Agent Offset (torus-correct). | `I: Agent` (int) / `O: X` `O: Y` (float) | Multi-output |
