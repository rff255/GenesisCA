...
| 8 | `loop` | Loop | Repeat flow N times. | `I: DO` (flow) `I: COUNT` (int) / `O: DONE` `O: BODY` (flow) `O: INDEX` (int) | DONE is the FIRST output (top); runs after all iterations. INDEX = the 0-based iteration counter, only valid inside the BODY chain (all targets: cells, agents, overseer) |
