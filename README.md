...
- **New nodes** — `Get/Set Orientation`, `Get Facing Orientation`, `Get Neighbor Orientation By Index`, `Set Facing Orientation` / `Set Neighbor Orientation By Index` (async-only), `Get Facing Labels` / `Get All Facing Labels` (resolve the face labels touching at an encounter, accounting for both cells&apos; orientations and patterns), `Lookup Interaction` / `Interaction Table Map` (index a table by two face labels), and `Move Self To Neighbor` (atomic move-into-vacancy). Hidden from the palette when the feature is off. Fully supported on JS, WASM, and WebGPU (only the two async-only orientation writers are unavailable on WebGPU, which is synchronous-only).
