...
- **Agent Output Mappings (their own views)** — agents get their **own Attribute→Color output mappings**, the off-lattice analog of the grid's: pick an **agent attribute → colour** (a colour scale or per-tag palette) in the Mappings panel instead of hand-wiring Set Cell Looks in the Behaviour Step. When both the grid and the agents define views, the simulator's viewer bar becomes a **two-layer selection** (a *Cells* row + an *Agents* row), so you can switch each layer's view independently.
...
- **Vector & Color value types** — bundle X / Y / Z onto one **vector** wire (or R / G / B / A onto one **color** wire) with **Make Vector / Break Vector** and **Make Color / Break Color** (the Unreal/Blender Make-Break pattern), and do whole-vector math with **Vector Op** (add / subtract / scale / dot / cross / length / normalize / distance / negate / lerp) — so you operate on a coordinate as a unit instead of touching each component. Apply Force has an optional **Vector input** mode that takes a force vector directly. (Composite-type nodes run on the JS engine; a model that uses them clamps to the Debug/Reference target.)
...
- [Node Reference](docs/NODES_REFERENCE.md) — full catalogue of all 107 node types (including the Bond-Graph Agents family), port schemas, and compile-time semantics, with Mermaid diagrams of common patterns.
