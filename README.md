...
- **Agent sprites** — draw an agent as a **static image, an animated GIF, an image sequence, or a sliced sprite sheet** instead of a circle. Import sprites in the Mappings panel — a single image/GIF, **several images as one animation** (frame sequence), or **one grid image sliced into frames** (RPGMaker-style sprite sheet, with cols/rows/count + margin/spacing). Each sprite can **rotate** — set the art's default facing on a compass dial and enable **Orient to velocity** so it auto-points along the agent's heading (plus a fixed offset) — and **remove a background color** (chroma-key a magenta/green screen to transparency, with a tolerance). A **Set Agent Sprite** node controls the exhibition from your rule graph, with independently-tickable **change sprite / set frame / set speed** options (speed in frames per simulation step — **negative reverses**, 0 holds). Playback is **driven by the agent's logic** (e.g. *while moving, play the walk cycle; while idle, hold*), not a manual transport.
