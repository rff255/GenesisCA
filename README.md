...
- **v1 scope** — the agent **engine** runs on JavaScript, but the **cell field may use the WASM fast path** (the field is shared memory the agent loop reads directly, so a field-heavy morphogenesis model gets accelerated diffusion while agents stay JS); WebGPU and 3D agents are the next milestones. The grid/lattice path is unchanged on all three targets.
