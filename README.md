...
- **Independent grid/agent compile targets** — the **grid** can run on any target (JS / WASM / **WebGPU**) while the agent engine stays on JavaScript. A WASM grid shares memory with the agents directly; a **WebGPU grid** (e.g. chemical diffusion on the GPU) works via a per-step field-readback bridge, so a field-heavy morphogenesis model gets accelerated diffusion while agents stay JS. The **Agent Compile Target** (Properties → Bond-Graph Agents) is independent of the grid's; WASM/WebGPU *agent* loops are the next milestone (Phase F). The grid/lattice path is unchanged on all three targets; 3D agents are the final milestone.
