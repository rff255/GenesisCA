...
- **Graph Editor** — 50+ node types across 8 categories (event, flow, data, logic, aggregation, output, color, indicators), with RMB pan, scroll zoom, and snap-to-grid. Includes Switch (flow routing by conditions or value), For Each In Array (iterate any typed array), an **Expression** node (type a math formula — `+ - * / % ^`, `sqrt abs floor ceil round min max pow mod` — instead of wiring up many Math nodes, ideal for equation-heavy models), Proportion Map and Color Interpolation (with selectable curves: Linear, Smoothstep, Ease-In/Out Quadratic, Exponential, Logarithmic), Interpolation, Aggregate (multi-input math), neighborhood tag-based access nodes, NeighborIndex bootstrap (Get All Neighbor Indexes), constructors, Flip, Pick Random Neighbor / Pick N Random Neighbors, generic Array Element / Array Length, and a **Stop Event** node that pauses the simulator with a user-defined message when its flow input fires.
...
- **Canvas controls** — LMB=brush, RMB=pan, MMB=toggle autoscroll (anchor + speed-by-distance pan; Esc to exit), scroll=zoom, Ctrl+LMB drag=resize brush, Ctrl+wheel=cycle input mappings, Shift+RMB=open in-page color picker at the cursor, Shift+LMB=open Inspect Cell popup (live attribute readout + RGB; draggable and stackable for comparing cells; hover to highlight the linked cell). The stats overlay shows the cell coordinates under the cursor (or the brush footprint range when brush > 1×1)
