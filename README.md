...
- **Agents sense each other &amp; the graph authors the forces** — **Get Nearby Agents** returns the flock-mates within a radius (the agent analog of *Get Neighbors*); read each with **Get Agent Position / Offset / Attribute / Radius / Get Velocity**, then steer with **Apply Force** so the rule graph *authors the physics* (separation/alignment/cohesion, chemotaxis up a gradient, propulsion) on top of (or instead of) the engine soft-sphere. **Get Agent Offset** gives the *torus-shortest* displacement to a neighbour, so flocking stays correct across a wrapped boundary (use it, not raw position subtraction). A **momentum** setting adds flocking inertia, **Get Curvature** measures local membrane curvature, and **Set Agent Attribute** signals a neighbour.
...
- **Graph-authored spawning** — an **Agent Init Event** root runs once per Reset; loop inside it and spawn the initial population with **Create Agent** (allocate a staged agent → a handle) &rarr; **Set Agent Position / Radius / Type** (or Set Agent Attribute) &rarr; **Add Agent To World** (commit it). So a whole grid, blob, or scattered swarm is laid out by the rule graph rather than a fixed seeder.
- **Config panel** — the **Bond-Graph Agents** block (shown when Agents is on) sets agent/bond ceilings, seed count + pattern (compact blob or scattered), default radius, the soft-sphere force law (repulsion, adhesion, interaction range, drag, time step &mdash; auto-clamped for stability, growth rate), the flocking knobs (momentum, max speed, custom-forces-only, neighbour query radius), and bonding (auto-bond, stiffness, form/break distances). Capacity ceilings are over-allocated and **reject on overflow, never wrap**.
- **Ships as six working samples** — **Morphogenesis — Growing Tissue** (12 → ~1500 cells dividing along the tension axis); **Morphogenesis — Differential Tissue** (asymmetric division + a maturity gradient + contact inhibition = cell *specialization*, a self-limiting tissue); **Boids — Flocking** (separation/alignment/cohesion via Get Nearby Agents + Apply Force); **Chemotaxis — Aggregation** (slime-mould: secrete a chemical, the grid CA diffuses it, agents climb the gradient and aggregate); **Game of Life on Agents** (Conway's totalistic rule running on a grid of agents spawned by the Agent Init Event &mdash; a genericity proof that a classic lattice CA is just an agent rule); and **Ant Necrophoresis** (stigmergy: ants random-walk, sense local corpse density via the field bridge, and pick up isolated bodies / drop them on clusters, so corpse piles self-organize).
- **3D agents** — agents simulate and render in a **3D volume** too (not just 2D): they're pushed by 3D forces, **divide along their 3D tension axis**, and draw as shaded **sphere impostors with bond tubes** in the voxel viewport, with a 3D agent brush + inspector. A 2D agent model keeps the byte-identical Canvas2D overlay.
- **Independent grid/agent execution model** — the agent engine's **compile target** AND **update synchronicity** are set in Properties → Bond-Graph Agents, *independently of the grid's*, so a **synchronous grid rule can run with asynchronous agents, and vice versa**. **Compile Target:** **JavaScript** (full coverage) or **WebAssembly** (live for a growing node subset; falls back to JS otherwise) — WebGPU agents are coming. **Update Mode:** **Asynchronous** (a *Set Agent Attribute* to a neighbour is immediately visible this step) or **Synchronous** (every agent reads the previous step; writes swap in at the step's end — the snapshot/parallel semantics the GPU path needs). The **grid** independently runs on JS / WASM / **WebGPU**: a WASM grid shares memory with the agents directly, and a **WebGPU grid** (e.g. GPU chemical diffusion) feeds the agents via a per-step field-readback bridge. The grid/lattice path is unchanged on all three targets.
...
- [Node Reference](docs/NODES_REFERENCE.md) — full catalogue of all 108 node types (including the Bond-Graph Agents family), port schemas, and compile-time semantics, with Mermaid diagrams of common patterns.
