...
- **Graph Editor** — 49+ node types across 8 categories (event, flow, data, logic, aggregation, output, color, indicators), with RMB pan, scroll zoom, and snap-to-grid. Includes Switch (flow routing by conditions or value), For Each In Array (iterate any typed array), Proportion Map and Color Interpolation (with selectable curves: Linear, Smoothstep, Ease-In/Out Quadratic, Exponential, Logarithmic), Interpolation, Aggregate (multi-input math), neighborhood tag-based access nodes, NeighborIndex bootstrap (Get All Neighbor Indexes), constructors, Flip, Pick Random Neighbor / Pick N Random Neighbors, generic Array Element / Array Length, and a **Stop Event** node that pauses the simulator with a user-defined message when its flow input fires.
