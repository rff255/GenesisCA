...
- **Agent Capability Profiles** — the agent engine is **composed from opt-in capability modules** so a model shows only its paradigm's machinery. Pick a **preset** (Particle · Boids · Vivarium · Morphogenesis · Social Graph · CA-on-Agents) or toggle individual capabilities (Motion · Body · Collision · Bonds · Growth · Division · Lifespan · Population · Sensing · Orientation · Field coupling) in **Properties → Agent Capabilities**; the palette, the Behaviour-Step ports, and the agent Edit/inspector panels then **filter to what's enabled** — a social-graph author never sees `radius`, `force`, or `Divide Agent`, and dependencies auto-resolve (enabling Collision auto-enables Body). A live **per-agent footprint** readout makes the "cost of generality" legible. Existing models load with a tight, honest profile inferred from what their graph actually uses (a bonded, dividing tissue keeps everything; a pure flocking model shows only motion, body, and sensing).
