...
- **`Lookup Table` model attribute** — a (possibly **rectangular**) matrix with an independent **row key source** and **column key source**, each either **Custom labels** (an arbitrary set of row/column names you add/remove/rename right on the definition page — not tied to any attribute), a face-label palette (`['none', ...labels]`), a tag attribute (its options — e.g. empty/water/amphi), or **Single value (map)** — a one-element axis that collapses the table into a 1-D **map** keyed only by the other axis (no throwaway single-option tag attribute needed). The cell **value type** is selectable (Binary / Integer / Decimal / Tag — Decimal by default) with a type-appropriate per-cell editor. A pure tag×tag table needs no faces, so it works even with Variegated Cells off. Edit in the Attributes panel; live-tune in the Simulator like any other model attribute. Ships a **General 2D CA (Golly style)** sample — a generalized Life-like Born/Survive rule whose entire rule table is one Lookup Table with custom `0-8`×`Born/Survive` labels and a **Binary** value type (no helper attributes, no decimal placeholders), with 12 classic rules (Conway, Replicator, Diamoeba, Anneal, Morley, …) as presets.
