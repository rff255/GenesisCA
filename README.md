...
- **Map Image to Cells** — load an image (Open Image button, or **Ctrl+V** a clipboard image) into a **Mapping Cells** dialog: the source (left) with a draggable/resizable region box + cell-grid overlay, and a gridified preview (right). Choose the sampling cell size, average-vs-centre sampling, invert, binarize + threshold, the Colour→Attribute input mapping, and whether to **resize the grid to fit** or **paste centered**; or **use the manual input mapping** to paint the binarized-true cells with chosen attribute values. (2D grids; a 3D model keeps the classic 1px = 1 cell import.)
