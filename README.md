...
- **Two-graph editor** — enabling Agents adds a second rule graph (the **Agents** graph) alongside the **Cells** graph, switched by a **Cells / Agents** tab strip above the canvas behind the same editor. The palette swaps per tab: agent nodes appear only on Agents, grid/neighbourhood nodes only on Cells, and universal nodes (math, conditionals, Get/Set Attribute over the shared attributes, Set Cell Looks) in both. Agent nodes: **Behaviour Step** (the per-agent root) and **Division Event** roots; **Get Self Position / Get Radius / Get Bond Degree / Neighbour Density** reads; **Set Target Radius / Form Bond / Break Bond / For Each Bond / Divide Agent / Kill Agent** actions; and the field bridge **Sample Field / Field Gradient / Read Cells Under / Affect Cells Under / Secrete To Field**.
- **Config panel** — the **Bond-Graph Agents** block (shown when Agents is on) sets agent/bond ceilings, seed count and default radius, the soft-sphere force law (repulsion, adhesion, interaction range, drag, time step &mdash; auto-clamped for stability, growth rate), and bonding (auto-bond on/off, bond stiffness, form/break distances). Capacity ceilings are over-allocated and **reject on overflow, never wrap**.
- **Ships as a working sample** — the **Morphogenesis — Growing Tissue** model: 12 seed cells grow into a ~1500-cell bonded tissue that divides along the tension axis.
- **v1 scope** — the agent engine runs on the **JavaScript (Debug / Reference)** compile target (an agents model is compiled there automatically) and is **2D** for now; WASM/WebGPU agent compilation and 3D agents are the next milestones. The grid/lattice path is unchanged on all three targets.
