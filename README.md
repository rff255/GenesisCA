...
- **3D neighbourhoods** — the Neighborhoods panel swaps the flat grid for a **parametric** builder (Moore / von Neumann / Ball / Range-N / Shell / Ring / Disk, with an L∞/L1/L2 metric) plus a **slice-stack editor** (step through Z layers, toggle cells, three axis-plane mirrors, per-cell tags). The 2-axis NeighborIndex node family is hidden in 3D — use Get Neighbors Attribute / Get Neighbor Attribute by Tag.
- **WebGL2 voxel renderer** — the simulator renders 3D models as instanced cubes (only live, non-transparent cells are drawn): **orbit** the camera (LMB-drag / wheel / Shift-drag pan), pull a **clip/slice plane** to see inside, and Shift-click to inspect or click to paint a single voxel. **Authorable alpha** (a new `A` input on Set Cell Looks, default opaque) lets dead cells go transparent so the structure shows through; an opt-in alpha-blend toggle composites translucent cells.
