...
// max dims the model can offer (drives the UI 2D/3D option gating)
eq('model-dims 2D', m.vectorDimsForModel({ properties: { dimension: '2d', gridDepth: 1 } }), 2);
eq('model-dims 3D', m.vectorDimsForModel({ properties: { dimension: '3d', gridDepth: 8 } }), 3);
eq('model-dims 3d-1-layer', m.vectorDimsForModel({ properties: { dimension: '3d', gridDepth: 1 } }), 2); // is3dModelLike needs depth>1
eq('model-dims null', m.vectorDimsForModel(null), 2);

// per-attribute dims (explicit vectorDims, both offered in a 3D model)
eq('attr-dims default', m.vectorDimsOf({ type: 'vector' }), 2);
eq('attr-dims 2', m.vectorDimsOf({ type: 'vector', vectorDims: 2 }), 2);
eq('attr-dims 3', m.vectorDimsOf({ type: 'vector', vectorDims: 3 }), 3);
eq('attr-dims null', m.vectorDimsOf(null), 2);
...
eq('expand identity', m.expandVectorAttributes(scalars) === scalars, true);

// expandVectorAttributes — a 2D vector cell FIELD attr (agentAccess inherited) + boundary split
const attrs = [
  { id: 'flow', name: 'Flow', type: 'vector', vectorDims: 2, description: 'field', isModelAttribute: false, defaultValue: '1,-2', boundaryValue: '0,0', agentAccess: 'read' },
  { id: 'x', name: 'X', type: 'integer', description: '', isModelAttribute: false, defaultValue: '0' },
];
const expanded2 = m.expandVectorAttributes(attrs);
...
// per-attr dims: a 2D vector AND a 3D vector in the SAME (3D) model
const mixed = m.expandVectorAttributes([
  { id: 'h', name: 'H', type: 'vector', vectorDims: 2, description: '', isModelAttribute: false, defaultValue: '1,0' },
  { id: 'v', name: 'V', type: 'vector', vectorDims: 3, description: '', isModelAttribute: false, defaultValue: '1,2,3' },
]);
eq('mixed count', mixed.length, 5); // 2 + 3
eq('mixed 2D ids', mixed.slice(0, 2).map(a => a.id), ['h_vx', 'h_vy']);
eq('mixed 3D ids', mixed.slice(2).map(a => a.id), ['v_vx', 'v_vy', 'v_vz']);
eq('mixed 3D defaults', mixed.slice(2).map(a => a.defaultValue), ['1', '2', '3']);
eq('mixed no-boundary', mixed[0].boundaryValue, undefined);
