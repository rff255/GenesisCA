...
const model = {
  ...
  properties: {
    ...
    ruleDescription:
      ...
      + 'DATA SOURCES — REPLICATE OR RETARGET. All three layers are free, open data, downloadable with no account:\n'
      + '- LAND COVER: ESA WorldCover 2021 v200 (CC BY 4.0), 3 x 3 deg GeoTIFF tiles named by their SW corner in '
      + '3-degree steps (browse the map at esa-worldcover.org). This model reads S09W036:\n'
      + '  https://esa-worldcover.s3.eu-central-1.amazonaws.com/v200/2021/map/ESA_WorldCover_10m_2021_v200_S09W036_Map.tif\n'
      + '- ELEVATION (for the slope layer): Copernicus DEM GLO-30 (ESA/Airbus, free with attribution), 1 x 1 deg '
      + 'GeoTIFF tiles on the AWS Open Data bucket, named by SW corner — this model reads S09/W035:\n'
      + '  https://copernicus-dem-30m.s3.amazonaws.com/Copernicus_DSM_COG_10_S09_00_W035_00_DEM/Copernicus_DSM_COG_10_S09_00_W035_00_DEM.tif\n'
      + '- ROADS: OpenStreetMap (ODbL) via the Overpass API — the query asks for highway=motorway|trunk|primary|'
      + 'secondary|tertiary ways with "out geom" inside the bounding box. Try it interactively at overpass-turbo.eu, '
      + 'which can export the result as GeoJSON.\n'
      + 'TO GROW A DIFFERENT CITY, two paths. (1) The generator (recommended): edit the window (WEST/SOUTH) and the '
      + 'two tile URLs at the top of scripts/gen-urban-recife.mjs in the GenesisCA repository and re-run it with Node '
      + '— it range-reads only the window (a few hundred KB, cached under scripts/geodata-cache/), fetches the roads, '
      + 'and rebuilds every layer INCLUDING the two derived ones (`Distance to road` is a breadth-first search from '
      + 'the rasterised network and `Slope` is Horn\'s gradient over the DEM), plus the basemap and thumbnail. '
      + '(2) Partly in the app: the raw layers import directly — "Import GeoTIFF..." lands the WorldCover classes '
      + 'into `landUse` (Nearest), and "Import GeoJSON..." burns an overpass-turbo road export in as the Road class — '
      + 'but the app does not derive distance fields or slope, so `roadDist` and `slope` would stay flat unless you '
      + 'compute them elsewhere (e.g. export from a GIS as rasters and import them as extra GeoTIFF layers).',
    instructions:
