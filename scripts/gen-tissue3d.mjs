#!/usr/bin/env node
/**
 * Generates public/models/Morphogenesis - 3D Tissue.gcaproj — the 3D analog of
 * gen-tissue.mjs (Differential Tissue), the PR2 verification vehicle for the 3D
 * agent ENGINE (the capstone's Life3D-equivalent: the model that can only exist
 * once the 3D engine does).
 *
 * Same developmental-tissue rule as the 2D version: each cell carries a
 * `maturity` (lineage depth); division is ASYMMETRIC (the Division Event):
 * daughter 0 self-renews (keeps maturity — a stem pool), daughter 1 advances.
 * A cell divides only while grown AND uncrowded (Neighbour Density < cap —
 * contact inhibition) AND uncommitted (maturity < MAX). Colour encodes the
 * differentiation stage. The ONLY difference is the WORLD: a 3D volume
 * (dimension '3d', gridDepth), seeded as a 3D ball, dividing along the engine's
 * 3×3 tension axis (axisSource 'tension') — so a glued cluster elongates +
 * divides along its mechanical axis in 3D, growing a connected 3D tissue.
 *
 * Built programmatically (mirrors gen-tissue.mjs). Re-run: node scripts/gen-tissue3d.mjs
 */
...
const OUT = resolve(__dirname, '../public/models/Morphogenesis - 3D Tissue.gcaproj');
...
const DENSITY_CAP = 10;   // contact inhibition: no division when this crowded (higher in 3D — more neighbours)

const GRID = 60;          // W = H
const DEPTH = 40;         // gridDepth — the 3D volume axis (>1 ⇒ the 3D engine)
...
// axisSource 'tension' = the engine's 3D tension-axis eigensolve (B6).
...
const model = {
  ...
  properties: {
    ...
    author: '', projectAuthor: '', tags: ['agents', 'morphogenesis', '3d', 'differentiation', 'stem cells', 'tissue'],
    dimension: '3d', gridWidth: GRID, gridHeight: GRID, gridDepth: DEPTH,
    topology: '2d-grid', boundaryTreatment: 'constant',
    useWasm: false, useWebGPU: false,
  },
  topologyMode: { gridCells: true, agents: true },
  centerBased: {
    enabled: true, agentTarget: 'webgpu', maxAgents: 4000, maxBonds: 12, worldWidth: GRID, worldHeight: GRID,
