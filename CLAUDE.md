...
- Neighborhood shape tools (Point/Circle/Ring/Line in the grid editor): tool + staged clicks + hover live in component state; pure helpers `annulusCells` (euclidean disc/ring, clipped to margin box) / `lineCells` (integer Bresenham) / `shapeCells` compute the affected set; everything applies through `toggleCellsChanges` as ONE updateNeighborhood (toggle semantics — covered active cells deactivate; a covered center flips `includeCentralCell`). Staged cells show a dashed accent outline (`.gridCellStaged`), the would-be shape previews under the hover (`.gridCellPreview`); right-click cancels staging (and falls through to the tag right-click only when nothing is staged).
