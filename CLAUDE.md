...
- **Spacebar quick-add**: Space (ModelerView capture-phase keydown) opens the Palette, focuses its search (`PaletteHandle.focusSearch()` via ref, same 50ms-timeout pattern as Ctrl+F/NodeExplorer), and freezes the cursor's flow position in `quickAddPosRef`. The Palette keeps a persistent keyboard selection over its flat visible-item list (nodes → default macros → project macros): ↑/↓ moves it (wraps), typing re-anchors it to the best LABEL match (first label-prefix, else label-substring, else item 0 — the list itself also includes description matches which would otherwise win by category order), mouse-move syncs it, Enter commits → `onQuickAdd(payload)` → ModelerView adds via `quickAddApi.addFromPalette` at the frozen position and collapses the panel; Esc closes. `quickAddApi` lives in [graphState.ts](src/modeler/vpl/graphState.ts) (GraphEditor registers `{getCursorFlowPos, addFromPalette}` on mount — `addFromPalette` = `spawnPalettePayload`, the shared palette-payload spawner also used by the drop handler). Space no longer toggles the palette closed (re-press re-arms quick-add); Esc/chevron/tab-click close it.
