...
Pre-resolve (`preResolveMoveNodes` in `compile.ts`, unchanged) bakes each slot's attribute defaultValue into `_attr_${i}_default` config (string), normalising `'true'`/`'false'` → `'1'`/`'0'` — used for the Defaults option. WASM reuses the read-modify-write-at-writeOffset pattern (mirrors `updateAttribute`); swap uses temp locals.

CaNode UI: Operation dropdown + Source-cell dropdown (hidden for swap) + attribute-slot rows (dropdown + `−`, `+ Slot`) + `Include Orientation` checkbox. One node replaces the old multi-node Amphiphile move sequence.
