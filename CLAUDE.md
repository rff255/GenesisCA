...
- **Legend click-to-hide (Lines/Stack)**: clicking a legend entry dims/hides that series (line-through + opacity 0.4); click again to restore. Runtime, display-only state `indicatorHiddenCategories: Record<indicatorId, Set<category>>` owned in `SimulatorView`, persisted in `genesisca_sim_settings` as `Record<id, string[]>`, threaded via `IndicatorDisplay` to both chart components (`hidden` + `onToggleCategory` props). The charts skip hidden series in the scaling + draw loops (Stack recomputes its cumulative bands + yMax from visible series only; Lines rescales Y over visible). `colorFor()` stays keyed by index in the FULL sorted category list, so hiding one series never recolors the others. **Distinct from `trackedValues`** (Track Categories): `trackedValues` is a design-time, model-level filter applied in the worker (changes which data reaches the chart); legend-hide is a per-session UI toggle (NOT saved into `.gcaproj`).
