...
- **Detail editor in a SECOND left panel**: the Attributes / Local Variables / Neighborhoods / Mappings panels are master-detail. The LIST stays in the primary `PanelShell`; the selected item's editor renders in a separate `PanelShell` mounted between the primary panel and the graph (so the user never scrolls past the list to reach the editor). Selection is lifted out of each panel into `ModelerDetailContext` ([ModelerDetailContext.tsx](src/modeler/ModelerDetailContext.tsx)) — `selectedByPanel: Partial<Record<PanelId, string|null>>` owned by `ModelerView`, exposed via `useDetailSelection(panel)` (used like `useState`). Each panel component takes a `mode?: 'list'|'detail'` prop and renders only that half (the same component is mounted twice). Properties + Variegated are single forms (not split). `ModelerView` shows the detail panel only when `activePanel ∈ {attributes,variables,neighborhoods,mappings}` AND its selected item still resolves (so deleting an item / closing the detail panel's ear-tab clears the selection and hides it). Per-panel selection survives ActivityBar tab switches. **Neighborhoods was converted from index-based (`selectedIdx`) to id-based selection** to fit the shared id-keyed context. **Local Variables is now its OWN ActivityBar panel** (`'variables'` PanelId, **L** icon, between Attributes and Neighborhoods) — `VariablesPanelSection` was converted to the same `mode`/`useDetailSelection('variables')` master-detail shape and removed from the Attributes panel. Only `IndicatorsPanelSection` (in Properties) keeps its inline list+inspector.
...
`VariablesPanelSection` is its **own ActivityBar panel** (`'variables'` PanelId, **L** icon, between Attributes and Neighborhoods) — a master-detail panel like Attributes/Neighborhoods/Mappings (`mode` prop + `useDetailSelection('variables')`; the variable list lives in the primary panel, the inspector in the second detail panel). It was previously embedded in the Attributes tab. A `.sectionHelp` line carries a short description under the section title (styled to match the compile-target option descriptions: `0.66rem`, `#888`, non-italic). Inspector fields: name, description, kind, dataType, length, initialValue, tag attribute for tag-typed variables, delete button. `+ Variable` adds a new variable with sensible defaults (scalar float, initialValue 0). Drag-to-canvas not supported in v1.
