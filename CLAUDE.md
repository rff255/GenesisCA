...
- **Macro inside a `forEachInArray` body (JS-target hoisting bug, fixed):** a macro whose inputs depend on the loop `element`/`index` must compile INSIDE the loop (per-iteration). Two coupled gaps, both because **macro ports are dynamic** (`getNodeDef('macro').ports` is `[]`): (1) the element/index dependency BFS — `findElementDependents` (compile.ts) AND `forwardValueConsumersMulti` (sinkAnalysis.ts) — propagated forward only via a consumer's static `def.ports`, dead-ending at a macro; both now ALSO discover a consumer's output ports from the **edge map** (any edge whose `source.nodeId === consumerId`), so the walk traverses THROUGH a macro to its downstream consumers. (2) `inlineMacroValues` emitted straight to cell-top `valueLines`; it now routes every line through **`routeValueEmit(macroNodeId, …)`** (via the `emitMacroLine` helper) so the macro lands in `branchValueLines[emitScope]` and `flushBranchValues` flushes it into the right block. The `bodyTarget` approach does NOT work here because the macro is usually compiled during the `collectValueDeps` pre-pass (when `bodyTarget` is still null) — only `routeValueEmit`'s sink-scope routing survives that. A loop-invariant macro still scopes to CELL_TOP (unchanged). JS-target only: WASM/WebGPU `expandMacros` upfront so there's no opaque macro node at analysis time. Verified: Chromatography's "Calculate Weight[d]" macro (wired to forEach `element`+`index`) now emits inside the loop on JS, and a minimal `macro(index*10) → sum` repro yields 60/step (was NaN/hoisted before).
